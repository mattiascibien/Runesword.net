...
Module modDialog
	...
	Public bdMaxMonsPics As Short '= 12
	' [Titi 2.4.9b] Cause of the mixed up pictures bug in combat when more than 12 creatures
	...
	Public PicPortrait() As String
	Public PicFileTime() As Double
		...
		With frmMain
			...
			Select Case Style
				Case modGameGeneral.DLGTYPE.bdDlgNoReply, modGameGeneral.DLGTYPE.bdDlgItem
					.picConvo.Height = 146
				Case modGameGeneral.DLGTYPE.bdDlgExamItem
					.picConvo.Height = 212
	...
	Public Sub DialogClick(ByRef AtX As Short, ByRef AtY As Short, ByRef ButtonDown As Short)
		...
		Dim rc As Integer
		' Count the available buttons (compatibility issues here)
		For c = 0 To 4
			...
			For c = 1 To ButtonCnt
				If PointIn(AtX, AtY, .picConvo.Width - 6 - 96 * c, .picConvo.Height - 30, 90, 18) Then
					...
					If ButtonDown = False Then
						...
						If frmMain.picGrid.Visible = True And ButtonList(c - 1) = "Use" Then
							' if using an item during combat, close the box
							frmMain.picConvo.Visible = False
						End If
						' [Titi 2.6.1] DialogShow and DialogBuySell bug fix
						If ButtonList(c - 1) = "Done" Then
							frmMain.picConvo.Visible = False
							bDialog = False
						End If
					End If
	...
	Public Function DialogShow(ByRef CreatureX As Object, ByRef Text As String) As Short
		...
		With frmMain
			...
			Do Until ConvoAction > -1
				...
				' If Talk form was activated over a Dialog needs to become visible again.
	...
	Public Sub DialogShowFaceBrief(ByRef CreatureX As Creature)
		...
		With frmMain
			...
			ElseIf CreatureX.Name = "Whew! No Traps!" Then  ' [Titi 2.6.0]
				.ShowText(.picDialogBrief, 0, 12, .picDialogBrief.Width, 14, bdFontElixirWhite, "Whew! No Traps!", True, False)
				'UPGRADE_ISSUE: PictureBox property picFaces.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
				'UPGRADE_ISSUE: PictureBox property picDialogBrief.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
				rc = BitBlt(.picDialogBrief.hdc, X, Y, 66, 76, .picFaces.hdc, bdFaceNoTraps, 0, SRCCOPY)
	...
	Public Sub DialogBrief(ByRef CreatureX As Object, ByRef Text As String)
		...
		If Not Found Then
			...
			CreatureZ.Name = "DM"
			' [Titi 2.6.0] could also be the "Whew! No Traps!" picture
			'UPGRADE_WARNING: Couldn't resolve default property of object CreatureX. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If CreatureX = "Whew! No Traps!" Then
				CreatureZ.Name = "Whew! No Traps!"
			End If
		End If
	...
	Public Sub DialogBuySellClick(ByRef AtX As Short, ByRef AtY As Short, ByRef ButtonDown As Short)
		...
		ElseIf PointIn(AtX, AtY, 41, frmMain.picBuySell.ClientRectangle.Height - 30, 90, 18) And BuySelect > 0 Then 
			...
			If ButtonDown = False Then
				...
				Else
					DialogDM(CreatureSelling.Name & " doesn't have anything more in stock.")
				End If
			End If
	...
	Private Sub DialogBuySellTransaction(ByRef IsBuying As Short)
		...
		Dim NoFail As Short
		' [Titi 2.6.0] RT 5 if trying to sell from an empty inventory
		If (BuyList.Count() = 0 And IsBuying = True) Or (SellList.Count() = 0 And IsBuying = False) Then Exit Sub
		...
		If NoFail = True Then
			...
			If IsBuying = True Then
				...
				' [Titi 2.4.9b] if the first item was sold, you had to reselect one
				' BuySelect = BuySelect - 1
				BuySelect = BuySelect + CShort(BuySelect <> 1)
			...
			Else
				...
				' [Titi 2.4.9b] if the first item was bought, you had to reselect one
				' SellSelect = SellSelect - 1
				SellSelect = SellSelect + CShort(SellSelect <> 1)
			...
			If NoFail = True Then
				...
				End If
			End If
			' [Titi 2.4.9b] even if the item is too high in level, it's known because the merchant gave the necessary explanations
			ItemNow.Examined = True
			' Redisplay
			DialogBuySellShow()
		End If
	End Sub
	...
	Public Sub DialogBuySellShow()
		...
		With frmMain
			...
			.ShowText(.picBuySell, 351, 12, 284, 14, bdFontElixirWhite, Left(CreatureWithTurn.Name & Space(26 - c), 26 - c), 0, False)
			...
			If BuyList.Count() > 0 And BuyTop > 0 Then
				...
				For c = BuyTop To Least(BuyTop + 2, BuyList.Count())
					...
					End If
					.ShowText(.picBuySell, 115, 56 + 102 * n, 214, 14, Font, ItemX.NameText & " (min lvl " & ItemX.MinLevel & ")", True, False)
					.ShowText(.picBuySell, 115, 74 + 102 * n, 214, 42, Font, "Bulk " & ItemX.Bulk & "  Wgt " & ItemX.Weight, True, False)
					' [Titi 2.4.9b] Show size too
					Select Case ItemX.Size
						...
						Case 5
							strSize = "Huge"
					End Select
					strAll = " but all sizes can use"
					If (ItemX.SizeBit(9) And 512) = 0 Then strAll = " creatures only can use"
					.ShowText(.picBuySell, 115, 92 + 102 * n, 214, 42, Font, strSize & strAll, True, False)
					...
					Else
						Text = ""
					End If
					.ShowText(.picBuySell, 115, 110 + 102 * n, 214, 14, Font, Text, True, False)
					' [Titi 2.4.8] let's use the currency of the world!
					Text = Left(WorldNow.Money, InStr(WorldNow.Money, "|") - 1)
			...
			If SellList.Count() > 0 And SellTop > 0 Then
				...
				For c = SellTop To Least(SellTop + 2, SellList.Count())
					...
					Else
						.ShowText(.picBuySell, 357, 56 + 102 * n, 214, 14, Font, ItemX.NameText & " (min lvl " & ItemX.MinLevel & ")", True, False)
					End If
					...
					strAll = " but all sizes can use"
					If (ItemX.SizeBit(9) And 512) = 0 Then strAll = " creatures only can use"
					.ShowText(.picBuySell, 357, 92 + 102 * n, 214, 42, Font, strSize & strAll, True, False)
					If ItemX.WearType < 10 Then ' Potential Armor
						Text = "Protect " & ItemX.Resistance & "%"
					ElseIf ItemX.Damage > 0 Then  ' Potential Weapon
						Text = "Damage " & (ItemX.Damage - 1) Mod 5 + 1 & "d" & Int(((ItemX.Damage - 1) Mod 25) / 5) * 2 + 4
					...
					Else
						Text = ""
					End If
					.ShowText(.picBuySell, 357, 110 + 102 * n, 214, 14, Font, Text, True, False)
					' [Titi 2.4.8] let's use the currency of the world!
					Text = Left(WorldNow.Money, InStr(WorldNow.Money, "|") - 1)
					If ItemX.Value >= 2 Then Text = Text & "s"
	...
	Public Function DialogItem(ByRef ItemX As Item, ByRef Text As String, Optional ByRef FromExamineItem As Boolean = False) As Short
		Dim OldPointer, intDone As Short
		Dim rc As Integer
		Dim ItemSplit As Item
		...
		With frmMain
			...
			'UPGRADE_WARNING: Screen property Screen.MousePointer has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
			System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.Default
			DialogSetUp(IIf(FromExamineItem, modGameGeneral.DLGTYPE.bdDlgExamItem, modGameGeneral.DLGTYPE.bdDlgItem))
			.ShowText(.picConvo, 0, 12, .picConvo.Width, 14, bdFontElixirWhite, (ItemX.Name), True, False)
			.ShowText(.picConvo, 56, 34, 380, 102, bdFontNoxiousGold, IIf(ItemX.Capacity > 0, "Container * ", "") & "Bulk " & ItemX.Bulk & " * Weight " & ItemX.Weight & " * " & strSize, False, False)
			...
			If .picInventory.Visible = True Then
				If ItemX.Count > 1 Then
					...
					' [Titi 2.4.9b] We may want to split the item rather than use it!
					intDone = 1 : blnSplit = True
					If ItemX.Triggers.Count() > 0 Then
						DialogSetButton(3, "Use")
						.ShowButton(.picConvo, .picConvo.Width - 6 - 96 * 3, .picConvo.Height - 30, "Use", False)
						intDone = 2
					End If
				Else
			...
			ElseIf ItemX.Capacity > 0 Then 
				' [Titi 2.5.1] if it's a container and on the ground, just open it!
				.picConvo.Visible = False
				DialogItem = 0
				Exit Function
			End If
			...
			' [Titi 2.4.9b]
			If ConvoAction < intDone And .picInventory.Visible = True Then
				If ConvoAction = intDone - 1 And blnSplit Then
					DialogDM("You split the " & ItemX.Name & " into two piles.")
					ItemSplit = CreatureWithTurn.AddItem
					ItemSplit.Copy(ItemX)
					...
					ItemSplit.Count = Int(ItemSplit.Count / 2)
					ItemSplit.Value = Int(ItemSplit.Value / 2)
					ItemSplit.Weight = Int(ItemSplit.Weight / 2)
					ItemSplit.Bulk = Int(ItemSplit.Bulk / 2)
					ItemSplit.IsReady = False
					ItemSplit.InvSpot = 0
				Else
					.UseShow(ItemX)
				End If
