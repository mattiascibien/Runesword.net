...
Module modJournal
	...
	Public Sub JournalClick(ByRef AtX As Short, ByRef AtY As Short, ByRef ButtonDown As Short)
		...
		Else ' If not in add new journal entry mode
			...
			ElseIf PointIn(AtX, AtY, 25, 42, 81, 18) And ButtonDown = True Then 
				' Journal
				JournalMode = 0
				bAddMode = False
				Call PlayClickSnd(modIOFunc.ClickType.ifClick)
				JournalEntryLoad()
				JournalShow()
			ElseIf PointIn(AtX, AtY, 106, 42, 81, 18) And ButtonDown = True Then 
				' Quests
				JournalMode = 1
				Call PlayClickSnd(modIOFunc.ClickType.ifClick)
				JournalQuestsLoad()
				JournalShow()
			ElseIf PointIn(AtX, AtY, 187, 42, 81, 18) And ButtonDown = True Then 
				' Map
				JournalMode = 2
				Call PlayClickSnd(modIOFunc.ClickType.ifClick)
				JournalShow()
			ElseIf PointIn(AtX, AtY, 268, 42, 81, 18) And ButtonDown = True Then 
				' Combat Log
				JournalMode = 3
				Call PlayClickSnd(modIOFunc.ClickType.ifClick)
		...
		Else
			For c = 0 To 3
				If c = JournalMode Then
					...
					rc = BitBlt(frmMain.picJournal.hdc, 25 + c * 81, 42, 18, 18, frmMain.picMisc.hdc, 18, 18, SRCCOPY)
				Else
					'UPGRADE_ISSUE: PictureBox property picMisc.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
					'UPGRADE_ISSUE: PictureBox property picJournal.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
					rc = BitBlt(frmMain.picJournal.hdc, 25 + c * 81, 42, 18, 18, frmMain.picMisc.hdc, 0, 18, SRCCOPY)
				End If
			Next c
			frmMain.ShowText((frmMain.picJournal), 49, 45, 81, 14, bdFontNoxiousBlack, "Journal", False, False)
			frmMain.ShowText((frmMain.picJournal), 130, 45, 81, 14, bdFontNoxiousBlack, "Quests", False, False)
			frmMain.ShowText((frmMain.picJournal), 211, 45, 81, 14, bdFontNoxiousBlack, "Map", False, False)
			frmMain.ShowText((frmMain.picJournal), 292, 45, 81, 14, bdFontNoxiousBlack, "Combats", False, False)
			...
			Select Case JournalMode
				...
				Case 3 ' Combat log
					...
					'UPGRADE_WARNING: Couldn't resolve default property of object JournalList().Text. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					frmMain.ShowText((frmMain.picJournal), 36, 118, 276, 202, bdFontNoxiousBlack, BreakText(JournalList.Item(JournalTop).Text, 3), False, False)
					frmMain.ShowText((frmMain.picJournal), 0, 318, frmMain.picJournal.Width, 18, bdFontNoxiousBlack, "Combat " & JournalTop & " of " & JournalList.Count(), True, False)
					'UPGRADE_ISSUE: frmMain.picJournal was upgraded to a Panel, and cannot be coerced to a PictureBox. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="0FF1188E-27C0-4FED-842D-159C65894C9B"'
					frmMain.ScrollBarShow((frmMain.picJournal), 317, 81, 218, JournalTop, JournalList.Count(), 0)
				Case Else ' Journal
					frmMain.picMicroBox.Visible = False
	...
	Public Function CountQuests() As Short
		...
		intQuests = 0
		For	Each JournalX In Tome.Journals
			If Left(BreakText(JournalX.Text, 1), 5) = "Quest" Or Left(BreakText(JournalX.Text, 1), 3) = "Log" Then intQuests = intQuests + 1
		Next JournalX
	...
	Public Sub JournalCombatLoad(ByRef bInCombat As Boolean)
		Dim c, Found As Short
		Dim JournalX, JournalLog As Journal
		Dim strCreature, sText As String
		JournalList = New Collection
		Found = False
		For	Each JournalX In Tome.Journals
			If Left(BreakText(JournalX.Text, 1), 3) = "Log" Then
				If strCreature = "" Then
					strCreature = BreakText(JournalX.Text, 2)
					sText = JournalX.Text
				ElseIf strCreature = BreakText(JournalX.Text, 2) Then 
					' same creature, so append the data
					sText = sText & vbCr & BreakText(JournalX.Text, 3)
				Else
					strCreature = BreakText(JournalX.Text, 2)
					JournalLog = New Journal
					JournalLog.Text = sText
					JournalList.Add(JournalLog)
					sText = JournalX.Text
				End If
				Found = True
			End If
		...
		If Found = False Then
			JournalX = New Journal
			JournalX.Text = "Log|No combats logged.|"
			JournalList.Add(JournalX)
		Else
			JournalLog = New Journal
			JournalLog.Text = sText
	...
	Public Sub JournalEntryLoad()
		...
		Found = False
		For	Each JournalX In Tome.Journals
			If Left(BreakText(JournalX.Text, 1), 5) <> "Quest" And Left(BreakText(JournalX.Text, 1), 3) <> "Map" And Left(BreakText(JournalX.Text, 1), 3) <> "Log" Then
				JournalList.Add(JournalX)
				Found = True
			End If
		Next JournalX
