...
Friend Class frmMain
	...
	Public Sub mnuTrayMaximize_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles mnuTrayMaximize.Click
		...
		If oGameMusic.Status = IMCI.VIDEOSTATE.vsPAUSED Then oGameMusic.ResumePlay()
	End Sub
...
BackToPreviousState: 
		...
		If MapX.Hazard > 0 Then
			If 100 * Rnd() <= MapX.Hazard And (EncounterNow.Classification = 2 Or EncounterNow.Classification = 0) Then
				...
				For j = 1 To intMonsters
					...
					'UPGRADE_WARNING: Couldn't resolve default property of object MapX.Creatures.Item(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Public Sub UseShow(ByRef ItemToUse As Item)
		...
		If ItemToUse.Family = 3 Then
			InVar = ItemToUse.Damage - 1 : GlobalDamageRoll = 0
		...
		Do Until CreatureWithTurn.ActionPoints < 1
			...
			If Target < 0 Then
				If (CreatureTarget.Name = CreatureWithTurn.Name And CreatureTarget.Index = CreatureWithTurn.Index) Then
					...
					'UPGRADE_WARNING: Couldn't resolve default property of object CombatRange(CreatureWithTurn, CreatureTarget.Col, CreatureTarget.Row). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Private Function GetTileSetIndex(ByRef c As Short) As Short
		...
		For Index = Map.TileSets.Count() To 1 Step -1
			'UPGRADE_WARNING: Couldn't resolve default property of object Map.Tiles(L & c).TileSet. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object Map.TileSets(Index).Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If Map.TileSets.Item(Index).Index = Map.Tiles.Item("L" & c).TileSet Then
				' found the correspondance
				Exit For ' will keep the value of Index
			End If
		Next 
	...
	Public Sub CombatMove(ByRef InDirection As MOVEDIRECTION, ByRef Motive As MOVEMOTIVE, Optional ByRef Distance As Short = 0)
		...
		If NoFail = True Then
			...
			ChangeDirectionCol = CreatureWithTurn.Col : ChangeDirectionRow = CreatureWithTurn.Row
			' Find path to Target
	...
	Public Sub CombatStart()
		...
		DialogSetUp(modGameGeneral.DLGTYPE.bdDlgNoReply) ' [Titi 2.4.9b] added for display of number of creatures
		picConvo.Visible = False
		picTalk.Visible = False
		...
		If Tome.Creatures.Count + EncounterNow.Creatures.Count() > bdMaxMonsPics Then
			...
			ReDim Preserve PicFile(bdMaxMonsPics)
			ReDim Preserve PicPortrait(bdMaxMonsPics)
			ReDim Preserve PicFileTime(bdMaxMonsPics)
			'UPGRADE_WARNING: Couldn't resolve default property of object Tome.Creatures. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			For	Each CreatureX In Tome.Creatures
				LoadCreaturePic(CreatureX)
			Next CreatureX
		...
		If CreatureCount >= 1 Then
			...
			DialogShow(EncounterNow.Creatures.Item(1), IIf(EncounterNow.HaveEntered = True, "You fled... But the enemy is still here." & vbCrLf, "") & strCreatures(0) & IIf(CreatureCount = 1, " is", " are") & " getting ready for the battle.")
		End If
	...
	Private Sub TomeDo(ByRef Action As Short)
		...
		If Action = 7 Or Action = 3 Then
			Call PlayClickSnd(modIOFunc.ClickType.ifClickMainMenu)
		Else
	...
	Private Sub TomeSavesLoad()
		...
		If ScrollList.Count() > 0 Then
			...
			Do 
				...
				FileOpen(fp, ScrollList.Item(c).FullPath & "\name.dat", OpenMode.Binary)
				...
				Do 
					...
					ScrollList.Add(ScrollList.Item(c),  , c)
					iNbLines = iNbLines - 1
					c = c + 1
	...
	Private Sub TomeSavesDelete()
		...
		FileOpen(fp, ClipPath(ScrollList.Item(ScrollSelect).FileName) & "\name.dat", OpenMode.Binary)
		'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
		FileGet(fp, rc)
		SaveName = Space(rc)
		'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
		...
		If ConvoAction = 0 Then
			...
			Else
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().FileName. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				If oFileSys.CheckExists(ScrollList.Item(ScrollSelect).FileName, clsInOut.IOActionType.File, False) = False Then
...
ErrorHandler: 
	...
	Private Sub TomeSavesList(ByRef Index As Short)
		...
		End If
		' List saved Tomes
		n = 0 : ScrollSelect = -1 '0 [Titi 2.4.9b] grey "Delete" and "Replace" out if nothing selected
		For c = ScrollTop To Least(ScrollTop + 11, ScrollList.Count())
			...
			If IsBetween(c, Index - iNbLines, Index) Then
				...
				If TomeAction = bdTomeSaves Then
					'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					ShowText(picTomeNew, 332, 48, 217, 15, bdFontElixirBlack, ScrollList.Item(c).Name, True, True) 'False
				Else
				...
				' Show Screen Shot
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().FileName. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				If oFileSys.CheckExists(ClipPath(ScrollList.Item(c).FileName) & "\screen.bmp", clsInOut.IOActionType.File) Then
					'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().FileName. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().FileName. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				ShowText(picTomeNew, 324, 260, 230, 18, bdFontNoxiousBlack, VB6.Format(FileDateTime(ScrollList.Item(c).FileName), "ddd mmm dd yyyy hh:mm AM/PM"), True, True) 'False
				ScrollSelect = Index
			Else
	...
	Private Sub TomeNewList(ByRef Index As Short)
		...
		For c = 1 To TomeList.Count()
			If IsBetween(c, TomeListTop, Least(TomeListTop + 11, TomeList.Count())) Then
				...
				iNbLines = Int(Len(TomeList.Item(c).Name) / 25) + CShort(Len(TomeList.Item(c).Name) = 25) ' 25 characters per line
				' [Titi 2.6.0] tomes with long names have as many places as lines needed for display
				If IsBetween(c, Index - iNbLines, Index) Then
					'            If c = Index Then
					'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					ShowText(picTomeNew, 24, 68 + n * 22, 245, 22 * (1 + iNbLines), bdFontElixirGold, TomeList.Item(c).Name, True, True) 'False
				Else
					'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					ShowText(picTomeNew, 24, 68 + n * 22, 245, 22 * (1 + iNbLines), bdFontElixirWhite, TomeList.Item(c).Name, True, True) 'False
				End If
				n = n + 1 + iNbLines
			...
			If IsBetween(c, Index, Index + iNbLines) Then
				...
				Else
					...
					If TomeList.Item(c).IsPersistent = False And TomeList.Item(c).IsInPlay = True And TomeList.Item(c).OnAdventure = True Then ' [Titi 2.6.1]
	...
	Public Sub TomeGatherList()
		...
		If bFromMainMenu And TomeRoster.Count() > 0 Then
			...
			CreatureNow = TomeRoster.Item(TomeRosterSelect + CShort(TomeRosterSelect > TomeRoster.Count()))
		...
		For c = 0 To Least(2, TomeRoster.Count() - TomeRosterTop)
			...
			If TomeRoster.Item(TomeRosterTop + c).HPNow < 1 Then
				...
				ShowText(picTomeNew, 102, n * 86, 166, 14, bdFontNoxiousWhite, "Level " & TomeRoster.Item(TomeRosterTop + c).Level & " * DEAD", True, False)
			Else
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster(TomeRosterTop + c).HPNow. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster().Level. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				ShowText(picTomeNew, 102, n * 86, 166, 14, bdFontNoxiousWhite, "Level " & TomeRoster.Item(TomeRosterTop + c).Level & " * " & TomeRoster.Item(TomeRosterTop + c).HPNow & " of " & TomeRoster.Item(TomeRosterTop + c).HPMax, True, False)
			End If
			If WorldNow.Runes = True Then
				...
				ShowText(picTomeNew, 102, n * 86 + 16, 166, 14, bdFontNoxiousWhite, WorldNow.Magic & " * " & TomeRoster.Item(TomeRosterTop + c).Eternium & " of " & TomeRoster.Item(TomeRosterTop + c).EterniumMax, True, False)
			End If
			...
			'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster().Level. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			X = TomeRoster.Item(TomeRosterTop + c).Level
			...
			ShowText(picTomeNew, 102, n * 86 + i * 16, 166, 14, bdFontNoxiousWhite, "Exp pts " & TomeRoster.Item(TomeRosterTop + c).ExperiencePoints, True, False)
			i = i + 1
			...
			If i = 3 Then ShowText(picTomeNew, 102, n * 86 + i * 16, 166, 14, bdFontNoxiousWhite, IIf(TomeRoster.Item(TomeRosterTop + c).IsSpellCaster, "Spellcaster", "Cannot cast spells"), True, False)
		Next c
		' Set up buttons
		'UPGRADE_ISSUE: picTomeNew was upgraded to a Panel, and cannot be coerced to a PictureBox. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="0FF1188E-27C0-4FED-842D-159C65894C9B"'
	...
	Private Sub TomeNewBuild()
		...
		c = oFileSys.CheckExists(gAppPath & "\tomes\" & WorldNow.Name, clsInOut.IOActionType.Folder, True)
		c = FreeFile
	...
	Private Sub TomeNewClick(ByRef AtX As Short, ByRef AtY As Short, ByRef ButtonDown As Short, Optional ByRef btnRight As Boolean = False)
			...
			Else
				...
				If bFromMainMenu And iCount > 0 Then
					...
					If TurnsBeforeNextCamp = 0 Then
						...
						ReDim sItemList(TomeRoster.Item(TomeRosterSelect).Items.Count)
						iItemTop = 1
						'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster(TomeRosterSelect).Items. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						For	Each ItemX In TomeRoster.Item(TomeRosterSelect).Items
							n = n + 1
					...
					Else
						...
						ReDim sItemList(TomeRoster.Item(TomeRosterSelect).Triggers.Count)
						'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster(TomeRosterSelect).Triggers. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						For	Each TriggerX In TomeRoster.Item(TomeRosterSelect).Triggers
							If TriggerX.IsSkill Or TriggerX.TriggerType = bdOnCast Or TriggerX.TriggerType = bdOnCastTarget Then
								n = n + 1
				...
				Else
					Select Case TomeAction
						...
						Case bdTomePackage
							If TomeIndex > 0 Then
								'UPGRADE_WARNING: Couldn't resolve default property of object TomeList.Item().Text. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		...
		ElseIf PointIn(AtX, AtY, 549, 74, 18, 267) Then 
			...
			If TomeAction = bdTomeGather And iItemTop > 0 Then
				...
				'UPGRADE_ISSUE: picTomeNew was upgraded to a Panel, and cannot be coerced to a PictureBox. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="0FF1188E-27C0-4FED-842D-159C65894C9B"'
				If ScrollBarClick(AtX, AtY, ButtonDown, picTomeNew, 549, 74, 248, iItemTop, IIf(TurnsBeforeNextCamp = 0, TomeRoster.Item(TomeRosterSelect).Triggers.Count, TomeRoster.Item(TomeRosterSelect).Items.Count), 15) = True Then
					TomeGatherList()
				End If
			End If
						...
						If ConvoAction = 0 Then
							...
							If OnAdventureList(TomeRosterSelect - 1) <> "deleted savegame!" Then
								...
								If ConvoAction = 0 Then
									...
									'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
									UnlockFromTome(TomeRoster.Item(TomeRosterSelect).Name, TomeRosterSelect - 1)
								...
								DialogBrief(TomeRoster.Item(TomeRosterSelect), TomeRoster.Item(TomeRosterSelect).Name & " has now left " & OnAdventureList(TomeRosterSelect - 1) & "...")
							Else
							...
							'UPGRADE_WARNING: Couldn't resolve default property of object TomeRoster(TomeRosterSelect).Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							DialogBrief(TomeRoster.Item(TomeRosterSelect), TomeRoster.Item(TomeRosterSelect).Name & " is now available for other adventures!")
							'DialogHide
		...
		ElseIf PointIn(AtX, AtY, 500, 325, 18, 18) And TomeAction = bdTomeNew And TomeIndex > 1 And ButtonDown = True Then 
			...
			If TomeList.Item(TomeIndex).IsInPlay = True And TomeList.Item(TomeIndex).OnAdventure = True And TomeList.Item(TomeIndex).IsPersistent = False Then
				picTomeNew.Refresh()
				Call PlayClickSnd(modIOFunc.ClickType.ifClick)
		...
		ElseIf PointIn(AtX, AtY, 62, 331, 90, 18) And (TomeAction = bdTomeNew Or TomeAction = bdTomePackage) And TomeIndex > 1 Then 
			...
			Else
				...
				Else ' [Titi 2.6.0] delete package
					...
					Kill(TomeList.Item(TomeIndex).Text)
					TomeList.Remove(TomeIndex)
					TomeIndex = TomeIndex + CShort(TomeIndex > TomeList.Count())
					PackageList(TomeIndex)
				End If
			End If
		...
		If TomeSaveArea(PathName) Then
			...
			If Not SaveAsNew Then
				...
				If InStr(PathName, "Roster") > 0 Then
					...
					Call oFileSys.Copy(gAppPath & "\world\" & WorldNow.Name & "\" & Mid(Tome.FileName, 1, Len(Tome.FileName) - 4) & "*.*", PathName, clsInOut.IOActionType.File, True)
				End If
			...
			If intCountEncounterTarget >= 0 Then
				For	Each EncounterX In Map.Encounters
					For c = 0 To intCountEncounterTarget
						i = FreeFile
						If oFileSys.CheckExists(ListEncounterTarget(c).Name & ".rse", clsInOut.IOActionType.File, False) = True Then
							Call oFileSys.Delete(ListEncounterTarget(c).Name & ".rse", clsInOut.IOActionType.File, True)
						End If
			...
			Do Until FileName = ""
				...
				Else
					...
					For c = 1 To TomeList.Count()
						...
						'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						If UCase(TomeList.Item(c).Name) > UCase(TomeX.Name) Then
							'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							...
							Do 
								If iNbLines = 0 Then Exit Do ' no extra lines needed
								'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
								TomeList.Add(TomeX,  , "T" & TomeList.Item(c).Index)
								iNbLines = iNbLines - 1
							Loop Until iNbLines = 0
							Exit For
						End If
...
ErrorHandler: 
	...
	Private Sub PackageLoad()
		...
		If TomeList.Count() > 0 Then
			...
			Do 
				'UPGRADE_WARNING: Couldn't resolve default property of object TomeList().Text. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				sName = VB.Left(TomeList.Item(c).Text, Len(TomeList.Item(c).Text) - 4) ' get rid of the .rsp extension
				...
				Do 
					If iNbLines = 0 Then Exit Do ' no extra lines needed
					TomeList.Add(TomeList.Item(c),  , c)
					iNbLines = iNbLines - 1
					c = c + 1
				Loop Until iNbLines = 0
				c = c + 1
		...
		ElseIf PointIn(AtX, AtY, 26, 73, 276, 250) And ButtonDown = True Then 
			...
			If ScrollList3.Count() > 0 Then
				If c - ScrollTop + 1 <= ScrollList3.Count() Then ' [Titi 2.6.1] clicking below the last skill would cause RT9
					...
					c = ScrollList3.Item(c - ScrollTop + 1)
				End If
			End If
	...
	Private Sub MenuSkillShow(ByRef Index As Short)
		...
		For c = 1 To ScrollList.Count()
			' Show selected Skill
			'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If c = Index And DontDisplaySkill(ScrollList.Item(c)) = False Then
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				FileName = gDataPath & "\Graphics\Skills\" & ScrollList.Item(c).Name & ".bmp"
							...
							If StatementX.Statement = bdRuneStatement Then
								For i = 0 To 5
									If StatementX.B(i) > 0 Then
										...
										If Map.Runes(StatementX.B(i) - 1) > 0 Then
											...
											'UPGRADE_ISSUE: PictureBox property picTomeNew.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
											rc = BitBlt(picTomeNew.hdc, 340 + i * 32, 272, 32, 32, picRuneSet.hdc, StatementX.B(i) * 32 - System.Math.Abs(CInt(StatementX.B(i) > 10)) * 320 - 32, System.Math.Abs(CInt(StatementX.B(i) > 10)) * 32 + 128, SRCCOPY)
										ElseIf CreatureWithTurn.ActionPoints > 9 Then 
											'UPGRADE_ISSUE: PictureBox property picRuneSet.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
											'UPGRADE_ISSUE: PictureBox property picTomeNew.hdc was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
											rc = BitBlt(picTomeNew.hdc, 340 + i * 32, 272, 32, 32, picRuneSet.hdc, StatementX.B(i) * 32 - System.Math.Abs(CInt(StatementX.B(i) > 10)) * 320 - 32, System.Math.Abs(CInt(StatementX.B(i) > 10)) * 32 + 64, SRCCOPY)
											ShowText(picTomeNew, 328, 308, 220, 15, bdFontNoxiousBlack, "Not Enough Runes!", True, False)
										End If
									End If
								Next i
								Exit For
			...
			If IsBetween(c, ScrollTop, Least(ScrollTop + 9, ScrollList.Count())) Then
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				If DontDisplaySkill(ScrollList.Item(c)) = False Then
					If c = Index Then
						'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList().Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
...
ErrorHandler: 
		...
		If PointIn(AtX, AtY, 380, 364, 90, 18) Then
			...
			Else
				...
				For c = 1 To Worlds.Count()
					...
					If Worlds.Item(c).Name = "Eternia" Then
						WorldIndex = c
						WorldNow = Worlds.Item(c)
	...
	Private Function DoorNearByTile(ByRef Index As Short) As Short
		...
		If Index > 0 Then
			'UPGRADE_WARNING: Couldn't resolve default property of object Map.Tiles(L & Index).KeyBits. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object Map.Tiles(L & Index).Chance. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If Map.Tiles.Item("L" & Index).Chance > 0 Or Map.Tiles.Item("L" & Index).KeyBits > 0 Then
				DoorNearByTile = True
			End If
		End If
	End Function
	...
	Private Sub GameInterfaceLoad(ByRef blnKeepWorld As Short)
		...
		If blnKeepWorld = False Then sWorld = Worlds.Item(WorldIndex).Name
...
ErrorHandler: 
	...
	Private Sub GameInit()
		...
		ReDim PicFile(bdMaxMonsPics)
		ReDim PicPortrait(bdMaxMonsPics)
		ReDim PicFileTime(bdMaxMonsPics)
		' [rb] Added for 2.4.6
		...
		gLibPath = tmp
		If oFileSys.CheckExists(gLibPath & "\Library", clsInOut.IOActionType.Folder) Then
		...
		Else ' [Titi 2.6.0] command line feature added
			...
			If UCase(VB.Command()) Like "LASTSAVE" Then
				If ScrollList.Count() > 0 Then
					'UPGRADE_WARNING: Couldn't resolve default property of object ScrollList.Item().FileName. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			...
			ElseIf InStr(VB.Command(), "\") = 0 Or IsNumeric(VB.Right(VB.Command(), 1)) Then 
				...
				Else
					...
					picMainMenu.Visible = True
				End If
			ElseIf oFileSys.CheckExists(VB.Command(), clsInOut.IOActionType.File) = True Then 
...
Err_Handler: 
	...
	Private Function SorceryQueRune(ByRef CreatureX As Creature, ByRef RuneToQue As Short) As Short
		...
		Else
			...
			If Map.Runes(RuneToQue) < 1 Or RuneToQue < 0 Or c > 5 Then
				Call PlayClickSnd(modIOFunc.ClickType.ifClickPass)
				SorceryQueRune = False
				Exit Function
			End If
...
ErrorHandler: 
	...
	Private Sub TomeCopy(ByRef TomeX As Tome)
		...
		If TomeX.IsInPlay = False Or TomeX.IsReset = True Then
			...
			' [Titi 2.4.9b] Check if the \world folder under the \tomes exists. If not, create it.
			c = oFileSys.CheckExists(gAppPath & "\World\" & WorldNow.Name, clsInOut.IOActionType.Folder, True)
			c = FreeFile
			FileOpen(c, gAppPath & "\world\" & WorldNow.Name & "\" & TomeX.FileName, OpenMode.Binary)
			TomeX.OnAdventure = False
			TomeX.SaveToFile(c)
...
ErrorHandler: 
		...
		Else
			...
			ElseIf (EncounterNow.HaveEntered = False And Friendly = True) And CreatureCount > 0 Then 
				...
				Else
					...
					DialogShow(EncounterNow.Creatures.Item(1), "There are " & strCreatures(0) & " here.")
				End If
				'            DialogHide
				'            MenuNow = bdMenuEncounter
				'            DoAction ConvoAction
	...
	Private Sub FindPath(ByRef TomeX As Tome)
		Dim pxlist(CDbl(Map.Height + 1) * CDbl(Map.Width + 1)) As Short
		...
		mv = 1
		' Set all coordinates in our traveled map to large value (we're not moving there)
		For X = 0 To Map.Width : For Y = 0 To Map.Height
				lMap(X, Y) = 32700
			Next Y : Next X
		' But plop a zero where we're starting from (we are moving there)
		lMap(sx, sy) = 0
	...
	Private Sub picConvoList_MouseMove(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles picConvoList.MouseMove
		...
		If Delta > 0 Then
			...
			If modDialog.DialogStyle = modGameGeneral.DLGTYPE.bdDlgCreatureList Then
				TargetCreatureClick(340, (2 + 18) + 2, True) ' top of the scrollbar (+2 pixels to not click on the up arrow)
			ElseIf modDialog.DialogStyle = modGameGeneral.DLGTYPE.bdDlgItemList Then 
	...
	Private Sub tmrDialogBrief_Tick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles tmrDialogBrief.Tick
		...
		If DialogBriefSet.Count() > 0 And TimeToRead = 0 Then
			...
			ShowText(picDialogBrief, 90, 37, picDialogBrief.ClientRectangle.Width - 90, picDialogBrief.ClientRectangle.Height, bdFontNoxiousWhite, DialogBriefSet.Item(1).Text, False, True) ' [Titi 2.4.9b] allows several lines even in brief dialogs!
	...
	Private Sub ShowMonster(ByRef CreatureX As Creature)
		...
		Else
			...
			End Select
		End If
		DialogSetUp(modGameGeneral.DLGTYPE.bdDlgExamItem)
