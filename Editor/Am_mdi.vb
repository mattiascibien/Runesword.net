	...
	Public Sub EditDelete(ByRef Tag_Renamed As String, Optional ByRef bConfirmation As Boolean = False)
		...
		Select Case ParseTag(Tag_Renamed)
			...
			Case bdEditCreature, bdEditMapCreatures
				...
				If frmWorldSettings.Visible And HoldNode = -1 Then frmWorldSettings.mnuDelete_Click(Nothing, New System.EventArgs())
				'UPGRADE_WARNING: Couldn't resolve default property of object ObjectDelete.Items. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				For	Each ItemX In ObjectDelete.Items
					EditDelete(Tag_Renamed & "I" & ItemX.Index)
				Next ItemX
	...
	Public Sub OpenTome(Optional ByRef strRecent As String = "", Optional ByRef Index As Short = 0)
		...
		Else
			...
			If frmWorldSettings.Visible Then
				frmWorldSettings.btnQuit_Click(Nothing, New System.EventArgs())
				frmWorldSettings.Close()
			End If
...
ErrorHandler: 
	...
	Private Function InitGame() As Boolean
		...
		oErr.logError("InitGame: Started", CErrorHandler.ErrorLevel.ERR_DEBUG)
		...
		End If
		If Not oFileSys.CheckExists(gLibPath, clsInOut.IOActionType.Folder) Then
...
Err_Handler: 
		...
		ElseIf Err.Number = 91 Then 
			...
			Resume Next
		Else
			oErr.logError("InitGame:  Failed.", CErrorHandler.ErrorLevel.ERR_CRITICAL)
		End If
		InitGame = False
		'Stop
		Resume Exit_Sub
	...
	Private Sub frmMDI_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		...
		If VB.Command() <> "" Then
			...
			ElseIf oFileSys.CheckExists(VB.Command(), clsInOut.IOActionType.File) = True Then 
				OpenTome(VB.Command())
			ElseIf oFileSys.CheckExists(VB.Command() & ".tom", clsInOut.IOActionType.File) = True Then 
	...
	Public Sub mnuCreaExpl_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles mnuCreaExpl.Click
		...
		bFileCk = oFileSys.CheckExists(gLibPath, clsInOut.IOActionType.Folder, False)
		If bFileCk = True Then
			frmMonsExplorer.Show()
		Else
			'UPGRADE_ISSUE: Load statement is not supported. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="B530EFF2-3132-48F8-B8BC-D88AF543D321"'
			Load(frmErrorDialog)
			frmErrorDialog.lblMessage.Text = "Error : Library directory not found in " & gLibPath
			frmErrorDialog.Show()
		End If
	End Sub
		...
		With frmWorldSettings
			...
			If Tome.Name = "Untitled" Then
				...
				If .Dirty Then
					...
					If hndWorld = MsgBoxResult.Yes Then frmWorldSettings.btnSave_Click(Nothing, New System.EventArgs())
				End If
				EndGame(hndWorld)
				' load saved world
			...
			.txtWorldName.Text = Text_Renamed
			lResult = fReadValue(FileName, "World", "PictureFile", "S", "", Text_Renamed)
			WorldNow.PictureFile = Text_Renamed
			If oFileSys.CheckExists(gAppPath & "\Roster\" & WorldNow.Name & "\" & WorldNow.PictureFile, clsInOut.IOActionType.File) = False Then
				' [Titi 2.4.9] not found - either deleted or renamed - anyway default to blank map!
				.txtWorldMap.Text = gDataPath & "\Stock\blankmap.bmp"
			...
			.txtNumberKingdoms.Text = Text_Renamed
			lResult = fReadValue(FileName, "World", "Intro", "S", "", Text_Renamed)
			.txtIntroMusic.Text = Text_Renamed
			lResult = fReadValue(FileName, "World", "MusicFolder", "S", "", Text_Renamed)
			.txtMusicFolder.Text = Text_Renamed
			' [Titi 2.4.9b] added to allow a world specific magic system (not necessarily based on runes)
			lResult = fReadValue(FileName, "World", "Magic", "S", "Eternium", Text_Renamed)
			.txtMagicName.Text = Text_Renamed
			WorldNow.Magic = Text_Renamed
			lResult = fReadValue(FileName, "World", "Runes", "S", "0", Text_Renamed)
			.chkRunes.CheckState = Val(Text_Renamed)
			WorldNow.Runes = .chkRunes.CheckState
			lResult = fReadValue(FileName, "World", "Formula", "S", "4 x Will", Text_Renamed)
			WorldNow.Formula = Text_Renamed
			.lblFormulaBasis.Text = Mid(Text_Renamed, 3)
			.txtCoef.Text = CStr(Val(Text_Renamed))
			Text_Renamed = VB.Right(Text_Renamed, Len(Text_Renamed) - InStrRev(Text_Renamed, " "))
			For hndWorld = 0 To .cmbMagicBasis.Items.Count
				If VB6.GetItemString(.cmbMagicBasis, hndWorld) = Text_Renamed Then .cmbMagicBasis.SelectedIndex = hndWorld
			Next 
			lResult = fReadValue(FileName, "World", "MagicPerLevel", "S", "10", Text_Renamed)
			.txtMagicPerLevel.Text = Text_Renamed
			lResult = fReadValue(FileName, "World", "RandomizeMagic", "S", "0", Text_Renamed)
			.chkMagicPerLevel.CheckState = CShort(Val(Text_Renamed))
			...
			.txtMoney(0).Text = VB.Left(Text_Renamed, InStr(Text_Renamed, "|") - 1)
			.txtMoney(1).Text = VB.Right(Text_Renamed, Len(Text_Renamed) - InStr(Text_Renamed, "|"))
			...
			End If
			.txtPicDescName(0).Text = VB.Left(Text_Renamed, InStr(Text_Renamed, "|") - 1)
			.txtPicDescName(1).Text = VB.Right(Text_Renamed, Len(Text_Renamed) - InStr(Text_Renamed, "|"))
			WorldNow.PicDesc = Text_Renamed
			'UPGRADE_WARNING: Screen property Screen.MousePointer has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
			System.Windows.Forms.Cursor.Current = System.Windows.Forms.Cursors.WaitCursor
			intKingdoms = Val(.txtNumberKingdoms.Text)
			hndWorld = 0
			...
			.TabStrip1.Tabs.Add(hndWorld)
			.Show()
			.TabStrip1.Tabs(hndWorld).Caption = .txtWorldName.Text & " Magic and Stats"
			' [Titi 2.4.9] now, add the tab for comments
			.TabStrip1.Tabs.Add(hndWorld + 1)
			.Show()
			.TabStrip1.Tabs(hndWorld + 1).Caption = .txtWorldName.Text & " comments"
			.Dirty = False
			...
			.TabStrip1.Tabs.Add(hndWorld)
			.Show()
			.TabStrip1.Tabs(hndWorld).Caption = "Map of " & .txtWorldName.Text
...
ErrorHandler: 
	...
	Public Sub mnuOptionsWorldSettingsCreate_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles mnuOptionsWorldSettingsCreate.Click
		...
		With frmWorldSettings
			...
			.txtWorldMap.Text = gDataPath & "\Stock\blankmap.bmp"
			.txtPicDescName(0).Text = .txtWorldMap.Text
			.txtPicDescName(1).Text = .txtWorldMap.Text
			WorldNow.PicDesc = .txtPicDescName(0).Text & "|" & .txtPicDescName(1).Text
			WorldNow.Description = "No description available."
			.picKingdom = System.Drawing.Image.FromFile(.txtWorldMap.Text)
			.txtWorldName.Text = "New world"
			...
			.chkRunes.CheckState = System.Windows.Forms.CheckState.Checked
			WorldNow.MagicPerLevel = 10
			WorldNow.RandomizeMagic = False
			hndWorld = .TabStrip1.Tabs.Count
			.txtNumberKingdoms.Text = "1"
			.Show()
			.TabStrip1.Tabs(hndWorld).Caption = .txtWorldName.Text
				...
				If UCase(ext) = "BMP" Or UCase(ext) = "ALL" Then
					...
					For	Each EncounterX In ObjectX.Encounters
						If EncounterX.Wallpaper <> "" Then
							If ObjectType <> bdEditData Then
								frmList.lstList.Items.Add(EncounterX.Wallpaper)
							Else
			...
			Case bdEditMap
				If UCase(ext) = "BMP" Or UCase(ext) = "ALL" Then
					...
					If ObjectType <> bdEditData Then
						...
						frmList.lstList.Items.Add(ObjectX.PictureFile)
					Else
						'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.PictureFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			...
			Case bdEditCreature ', bdEditMapCreatures
				If UCase(ext) = "BMP" Or UCase(ext) = "ALL" Then
					...
					If ObjectX.PortraitFile <> "" Then
						If ObjectType <> bdEditData Then
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.PortraitFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							frmList.lstList.Items.Add(ObjectX.PortraitFile)
						Else
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.PortraitFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				...
				If UCase(ext) = "WAV" Or UCase(ext) = "ALL" Then
					...
					If ObjectX.MoveWAV <> "" Then
						If ObjectType <> bdEditData Then
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.MoveWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							frmList.lstList.Items.Add(ObjectX.MoveWAV)
						Else
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.MoveWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					...
					If ObjectX.AttackWAV <> "" Then
						If ObjectType <> bdEditData Then
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.AttackWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							frmList.lstList.Items.Add(ObjectX.AttackWAV)
						Else
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.AttackWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					...
					If ObjectX.HitWAV <> "" Then
						If ObjectType <> bdEditData Then
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.HitWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							frmList.lstList.Items.Add(ObjectX.HitWAV)
						Else
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.HitWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					...
					If ObjectX.DieWAV <> "" Then
						If ObjectType <> bdEditData Then
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.DieWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
							frmList.lstList.Items.Add(ObjectX.DieWAV)
						Else
							'UPGRADE_WARNING: Couldn't resolve default property of object ObjectX.DieWAV. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						...
						With StmtX
							...
							If .Statement = 41 Then
								...
								If BreakText(TextQuote(.Text), 2) <> "" Then
									If ObjectType <> bdEditData Then
										frmList.lstList.Items.Add(BreakText(TextQuote(.Text), 2))
									Else
							...
							ElseIf .Statement = 46 Then 
								...
								If TextQuote(.Text) <> "" Then
									If ObjectType <> bdEditData Then
										frmList.lstList.Items.Add(TextQuote(.Text))
									Else
	...
	Private Sub FindFile(ByRef ext As String)
		...
		If UCase(ext) = "ALL" Then
			frmList.Text = "List of all additional files"
		Else
			frmList.Text = "List of additional ." & ext & " files"
		End If
		strFormCaption = frmList.Text
		...
		For i = 1 To TreeViewX.Nodes.Count
			frmList.Text = strProgress & i & "/" & TreeViewX.Nodes.Count & " completed..."
			frmList.Refresh()
	...
	Public Sub CheckIfExist(ByRef blnCopy As Boolean, Optional ByRef bPackWorld As Boolean = False)
		...
		End If
		If blnCopy Then
			If oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\", clsInOut.IOActionType.Folder, True) Then
				Call oFileSys.Delete(strRequiredDataFilesTxtFile & "data", clsInOut.IOActionType.Folder, True)
				'                Call oFileSys.Delete(strFound, Folder, True)
			End If
			' create structure
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\creatures\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\cutscenes\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\portraits\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\items\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\tiles\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\effects\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\wallpapers\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\graphics\skills\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\sounds\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\music\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\music\adventure\", clsInOut.IOActionType.Folder, True)
			blnExists = oFileSys.CheckExists(strRequiredDataFilesTxtFile & "data\music\combat\", clsInOut.IOActionType.Folder, True)
		End If
		intItemPos = 0
		blnCopiedAtLeastOne = False
		While intItemPos <= frmList.lstList.Items.Count - 1
			blnExists = False
			strFound = UCase(VB6.GetItemString(frmList.lstList, intItemPos))
			'UPGRADE_WARNING: Dir has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
			...
			If blnExists = True Then
				...
				frmList.lstList.Items.RemoveAt(intItemPos)
				frmList.lstList.Refresh()
			Else
				' No - keep it and increase index
				intItemPos = intItemPos + 1
			End If
			If frmList.lstList.Items.Count > 0 Then
				intPercent = Int(intItemPos / frmList.lstList.Items.Count * 100)
			Else
				intPercent = 100
			End If
			frmList.Text = "Searching files: " & intPercent & "%"
			frmList.Refresh()
		End While
		frmList.Text = strFormCaption
		frmList.Refresh()
		Dim aParse() As String
		If frmList.lstList.Items.Count > 0 Then ' some files not found
				...
				If MsgBox("Do you want to create a package for the " & IIf(bPackWorld, "world?", "tome?"), CDbl(MsgBoxStyle.Question & MsgBoxStyle.YesNo)) = MsgBoxResult.Yes Then
					...
					If retval <> 0 Then
						...
						If retval <> 0 Then
							...
							Else
								Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
								Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\World\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
								'UPGRADE_WARNING: Couldn't resolve default property of object Tome.LoadPath. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
								...
								frmList.btnExit.PerformClick()
							End If
						Else
							' [Titi 2.6.0] now move the package to \Packing store (easier to trace)
							Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
							Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\Library\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
							'UPGRADE_WARNING: Couldn't resolve default property of object Tome.LoadPath. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					...
					Else
						...
						Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
						'UPGRADE_WARNING: Couldn't resolve default property of object Tome.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						Call oFileSys.CheckExists(gAppPath & "\Packing store\" & WorldNow.Name & "\" & Tome.Name & "\", clsInOut.IOActionType.Folder, True) ' create subfolder if necessary
						'UPGRADE_WARNING: Couldn't resolve default property of object Tome.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						'UPGRADE_WARNING: Couldn't resolve default property of object Tome.LoadPath. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		...
		'    End If
Exit_Sub: 
		Call oFileSys.Delete(strRequiredDataFilesTxtFile & "data", clsInOut.IOActionType.Folder, True) '[Titi 2.4.9b] moved here to delete the \Data subfolder even after an error or cancellation of the package creation.
		Exit Sub
ErrorHandler: 
		If Err.Number = 53 Then
	...
	Public Sub btnOpen_Click()
		...
		While Not EOF(hndList)
			...
			If VB.Left(strFound, 5) <> "-----" And VB.Left(strFound, 9) <> "List of ." Then
				frmList.lstList.Items.Add(strFound)
			End If
		End While
		FileClose(hndList)
		'UPGRADE_WARNING: Screen property Screen.MousePointer has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
	...
	Private Sub RemoveDuplicate(ByRef lstList As System.Windows.Forms.ListBox)
		...
		With lstList
			...
			Do While intItemIndex <= .Items.Count - 1
				...
				End If
				frmList.Text = strProgress & intPercent & "%"
				frmList.Refresh()
			Loop 
		End With
		'UPGRADE_WARNING: Screen property Screen.MousePointer has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
