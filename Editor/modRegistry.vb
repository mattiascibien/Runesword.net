...
Error_Renamed: 
	...
	Function EnumChildProc(ByVal lhWnd As Integer, ByVal lParam As Integer) As Integer
		...
		Dim WinClassBuf As New VB6.FixedLengthString(255)
		Dim WinTitleBuf As New VB6.FixedLengthString(255)
		Dim WinClass, WinTitle As String
		Dim WinRect As RECT
		Dim WinWidth, WinHeight As Integer
		retval = GetClassName(pthWnd, WinClassBuf.Value, 255)
		WinClass = StripNulls(WinClassBuf.Value) ' remove extra Nulls & spaces
		retval = GetWindowText(lhWnd, WinTitleBuf.Value, 255)
		WinTitle = StripNulls(WinTitleBuf.Value)
	...
	Function EnumThreadProc(ByVal lhWnd As Integer, ByVal lParam As Integer) As Integer
		...
		Dim WinClass, WinTitle As String
		On Error GoTo Error_Renamed
		retval = GetClassName(lhWnd, WinClassBuf.Value, 255)
		WinClass = StripNulls(WinClassBuf.Value) ' remove extra Nulls & spaces
		retval = GetWindowText(lhWnd, WinTitleBuf.Value, 255)
		WinTitle = StripNulls(WinTitleBuf.Value)
