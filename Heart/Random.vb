...
Module modDungeonMaker
	...
	Public Sub SearchFolders(ByRef Extension As String, ByRef DirName As String, ByRef CollectionX As Collection)
	...
	Public Sub SearchCommonFolder(ByRef Extension As String, ByRef DirName As String, ByRef CollectionX As Collection)
		Dim FileName, LastFile As String
		Dim FactoidX As Factoid
		' [Titi 2.5.1]Does a simple search through a given directory for a given file extension.
		' Results are added to the given collection.
		'UPGRADE_WARNING: Dir has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
		FileName = Dir(DirName & "\" & Extension, FileAttribute.Normal)
		Do Until FileName = ""
	...
	Public Sub UberWizMakeMap(ByRef UberWizX As UberWizard, ByRef MapSketchX As MapSketch, ByRef AreaX As Area)
		...
		' [Titi 2.5.1] Copy camping options
		MapX.CanCamp = MapSketchX.CanCamp
		' [Titi 2.6.0] Copy wandering monsters
		For c = 1 To MapSketchX.Creatures.Count()
			'UPGRADE_WARNING: Couldn't resolve default property of object MapSketchX.Creatures().Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			CreatureX = MapX.AddCreatureAsIndex(MapSketchX.Creatures.Item(c).Index)
			'UPGRADE_WARNING: Couldn't resolve default property of object MapSketchX.Creatures(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			CreatureX.Copy(MapSketchX.Creatures.Item(c))
		Next c
		MapX.Interval = MapSketchX.Interval
		MapX.Hazard = MapSketchX.Hazard
	...
	Public Sub InitUberWizMaps(ByRef UberWizX As UberWizard)
		...
		' [Titi 2.5.1] but only those set in the current world
		'    SearchFolders "*.tom", gAppPath & "\library", UberWizX.TomeFiles
		'    SearchFolders "*.tom", gAppPath & "\Library\Worlds\" & WorldNow.Name, UberWizX.TomeFiles
		...
		SearchFolders("*.tom", gLibPath & "\Worlds\" & WorldNow.Name, (UberWizX.TomeFiles))
		' [Titi 2.5.1] however, let's add the tomes not flagged as world specific ones
		'    SearchCommonFolder "*.tom", gAppPath & "\Library\Maps", UberWizX.TomeFiles
		SearchCommonFolder("*.tom", gLibPath & "\Maps", (UberWizX.TomeFiles))
		For c = 1 To UberWizX.TomeFiles.Count()
			...
			For	Each AreaX In TomeX.Areas
				...
				For	Each MapX In AreaX.Plot.Maps
					...
					' [Titi 2.5.1] Don't forget the .CanCamp property!
					MapSketchX.CanCamp = MapX.CanCamp
		...
		' [Titi 2.5.1] take only maps designed for the current world
		'    SearchFolders "*.rsm", gAppPath & "\library", UberWizX.MapFiles
		'    SearchFolders "*.rsm", gAppPath & "\Library\Worlds\" & WorldNow.Name, UberWizX.MapFiles
		'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		SearchFolders("*.rsm", gLibPath & "\Worlds\" & WorldNow.Name, (UberWizX.MapFiles))
		' [Titi 2.5.1] also include the generic maps
		'    SearchCommonFolder "*.rsm", gAppPath & "\Library\Maps", UberWizX.MapFiles
		SearchCommonFolder("*.rsm", gLibPath & "\Maps", (UberWizX.MapFiles))
	...
	Public Sub InitUberWizThemes(ByRef UberWizX As UberWizard)
		...
		Dim ThemeSketchX As ThemeSketch
		' Load up Themes
		' [Titi 2.5.1] provided they're designed for the current world
		'    SearchFolders "*.rsr", gAppPath & "\library", UberWizX.ThemeFiles
		'    SearchFolders "*.rsr", gAppPath & "\Library\Worlds\" & WorldNow.Name, UberWizX.ThemeFiles
		'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		SearchFolders("*.rsr", gLibPath & "\Worlds\" & WorldNow.Name, (UberWizX.ThemeFiles))
		' [Titi 2.5.1] also include the generic themes (recursive search but only in the common themes folder)
		'    SearchCommonFolder "*.rsr", gAppPath & "\Library\Themes", UberWizX.ThemeFiles
		SearchCommonFolder("*.rsr", gLibPath & "\Themes", (UberWizX.ThemeFiles))
	...
	Public Sub InitUberWizCreatures(ByRef UberWizX As UberWizard)
		...
		' [Titi 2.5.1] Only creatures defined for the current world
		'    SearchFolders "*.rsc", gAppPath & "\library", UberWizX.CreatureFiles
		'    SearchFolders "*.rsc", gAppPath & "\Library\Worlds\" & WorldNow.Name, UberWizX.CreatureFiles
		'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		SearchFolders("*.rsc", gLibPath & "\Worlds\" & WorldNow.Name, (UberWizX.CreatureFiles))
		' [Titi 2.5.1] also include the generic creatures (recursive search but only in the common creatures folder)
		'    SearchFolders "*.rsc", gAppPath & "\Library\Creatures", UberWizX.CreatureFiles
		SearchFolders("*.rsc", gLibPath & "\Creatures", (UberWizX.CreatureFiles))
	...
	Public Sub InitUberWizItems(ByRef UberWizX As UberWizard)
		...
		' [Titi 2.5.1] only include items designed for the current world
		'    SearchFolders "*.rsi", gAppPath & "\library", UberWizX.ItemFiles
		'    SearchFolders "*.rsi", gAppPath & "\Library\Worlds\" & WorldNow.Name, UberWizX.ItemFiles
		'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		SearchFolders("*.rsi", gLibPath & "\Worlds\" & WorldNow.Name, (UberWizX.ItemFiles))
		' [Titi 2.5.1] also include the generic items (recursive search but only in the common items folder)
		'    SearchFolders "*.rsi", gAppPath & "\Library\Items", UberWizX.ItemFiles
		SearchFolders("*.rsi", gLibPath & "\Items", (UberWizX.ItemFiles))
	...
	Public Function MakeCreatureName(ByRef Method As Short, ByRef WorldName As String) As String
		...
		Else
			...
			If oFileSys.CheckExists(FileName, File) Then
				...
				' [Titi 2.6.0] skip the runes section
				For i = 1 To 6
					Name = LineInput(hndFile)
				Next 
	...
	Private Sub SizeItem(ByRef ItemX As Item, ByRef intFrom As Short, ByRef intTo As Short)
		' [Titi 2.4.9b] size added
		Dim c As Short
		For c = 0 To 4
			ItemX.SizeBit(c + 4) = 0
		Next 
		ItemX.SizeBit(9) = Int(Rnd() + 0.2) ' any size can use the item if 1 (so 20% chance to happen)
		ItemX.SizeBit(intFrom + Int(Rnd() * (intTo - intFrom) + 0.5)) = 1
	End Sub
	...
	Public Sub MakeItem(ByRef ItemX As Item)
		...
		Select Case ItemX.Family
				...
				If Int(Rnd() * 20) > 10 Then
					Select Case Int(Rnd() * 13)
						Case 0 : ItemX.Name = "Long " & ItemX.Name
							...
							SizeItem(ItemX, 2, 4) 'only Normal, Large and Huge creatures can use
							ItemX.SizeBit(9) = 0
						Case 1 : ItemX.Name = "Short " & ItemX.Name
							...
							Else
								ItemX.Damage = 6
							End If
							SizeItem(ItemX, 0, 2) 'only Normal, Small and Tiny creatures can use
							ItemX.SizeBit(9) = 0
			...
			Case 11 ' Random Gems
				...
				ItemX.Family = 0
				ItemX.SizeBit(9) = 1 '[Titi 2.4.9b] no problem due to size
			...
			Case 12 ' Random Treasure
				Select Case Int(Rnd() * 3)
					...
					Case 1 ' Random Money
						...
						If LCase(Left(WorldNow.Money, InStr(WorldNow.Money, "|") - 1)) = "gold piece" Or LCase(Left(WorldNow.Money, InStr(WorldNow.Money, "|") - 1)) = "gold coin" Then
							Select Case Int(Rnd() * 4)
								Case 0
					...
					Case 2 ' Random Chest
						...
						ItemX.SizeBit(9) = 1 '[Titi 2.4.9b] a chest can be used by anybody. But it limits the size of the items it can contain!
						c = 0
			...
			Case 13 ' Random Junk
				ItemX.Family = 0
				ItemX.SizeBit(9) = 1 '[Titi 2.4.9b] junk is available to every creature!
