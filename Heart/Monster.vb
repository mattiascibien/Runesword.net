...
Friend Class Creature
	...
	' [Titi 2.5.1] information
	'UPGRADE_ISSUE: clsInfos object was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6B85A2A7-FE9F-4FBE-AA0C-CF11AC86A305"'
	Private myInfos As New clsInfos
	' Flags for small checks
	'   Bit0    On=Agressive (Can't Ignore when Encounter)
	'   Bit1    On=Friendly
	'   Bit2    On=DMControlled
	'   Bit3    On=Guard (Can't Take Treasure)
	'   Bit4    On=HadTurn
	...
	' [Titi 2.4.9b] To increase EterniumMax with the level
	...
	Public Property AddEternium() As Short
		Get
			AddEternium = myNotUsed2
		End Get
		Set(ByVal Value As Short)
			If Value < 255 Then
				myNotUsed2 = Value
	...
	Public ReadOnly Property EterniumMax() As Short
		Get
			...
			Dim sOP, sStat As String
			Dim iCoef, iStat As Short
			...
			If WorldNow.Formula <> vbNullString Then
				'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Formula. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				iCoef = Val(WorldNow.Formula)
				'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Formula. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				sOP = Mid(WorldNow.Formula, InStr(WorldNow.Formula, " ") + 1, 1)
				'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Formula. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				sStat = Right(WorldNow.Formula, Len(WorldNow.Formula) - InStrRev(WorldNow.Formula, " "))
				Select Case sStat
					Case "Strength"
						iStat = myStrength
					Case "Agility"
					...
					Case "Will"
						iStat = myWill
					Case Else
						' [Titi 2.4.9b] also use the possibly new abilities
						For	Each TriggerX In Me.Triggers
							If TriggerX.TriggerType = bdShowNewStat Then
								For	Each TriggerY In TriggerX.Triggers
									If sStat = TriggerY.Name Then
										iStat = TriggerY.VarA
									End If
								Next TriggerY
							End If
			...
			End If
			EterniumMax = EterniumMax + Me.Level + Me.AddEternium
	...
	Public Property PictureFile() As String
		...
		Set(ByVal Value As String)
			If Len(BreakText(myPictureFile, 2)) > 0 Then
				myPictureFile = Trim(Value) & "|" & BreakText(myPictureFile, 2) & "|" & BreakText(myPictureFile, 3)
			ElseIf Len(BreakText(myPictureFile, 3)) > 0 Then 
				myPictureFile = Trim(Value) & "||" & BreakText(myPictureFile, 3)
			Else
	...
	Public Property PortraitFile() As String
		...
		End Get
		Set(ByVal Value As String)
			If Len(BreakText(myPictureFile, 3)) > 0 Then
				myPictureFile = BreakText(myPictureFile, 1) & "|" & Trim(Value) & "|" & BreakText(myPictureFile, 2)
			Else
	...
	Public Property PictureWhenDead() As String
		Get
			PictureWhenDead = Trim(BreakText(myPictureFile, 3))
			If PictureWhenDead = vbNullString Then PictureWhenDead = "bones.bmp"
		End Get
		Set(ByVal Value As String)
			myPictureFile = BreakText(myPictureFile, 1) & "|" & BreakText(myPictureFile, 2) & "|" & Value
		End Set
	End Property
	...
	Public Property Money() As Integer
		...
		Set(ByVal Value As Integer)
			...
			If Value > Me.Money Then
				...
				ItemX.Value = c
				ItemX.Count = c
				ItemX.Weight = 0 'c [Titi 2.5.0] money should be weightless
				ItemX.Bulk = 0 'Greatest(CInt(c / 10), 1) [Titi 2.5.0] money should be bulkless
			...
			ElseIf Value < Me.Money Then 
				...
				c = Me.Money - Value
				For	Each ItemX In Me.Items
					'            If ItemX.RemoveMoney(CInt(c)) = True Then
					' [Titi 2.4.9b] Changed the way money was spent, otherwise the amount was deducted from every pile of gold in the inventory!
					If ItemX.Family = bdMoneyType Then
						Balance = ItemX.RemoveMoney(CShort(c))
						If Balance <= 0 Then
							Me.RemoveItem("I" & ItemX.Index)
							c = -Balance
						Else
							If Balance > 1 Then
								' Seller has to give back some change!
								Me.Money = Me.Money + Balance - 1
							End If
							' paid what we bought, so exit
							Exit Property
						End If
					End If
				Next ItemX
	...
	Public Sub RemoveItem(ByRef DeleteKey As String)
		...
		Exit Sub
ErrorHandler: 
		' [Titi 2.6.0] for compatibility with previous versions
		If Err.Number = 5 Then
			DeleteKey = Mid(DeleteKey, 2)
			Resume 
		End If
	End Sub
	...
	Public Sub Copy(ByRef FromCreature As Creature)
		...
		Next c
		myInfos = New clsInfos
		Me.Infos = FromCreature.Infos
ErrorHandler: 
		Err.Clear() ' [Titi 2.5.1] if an error occurs, it's because we tried to copy infos from a creature not having this new details
	End Sub
	...
	Public Sub ReadFromFile(ByRef FromFile As Short)
		...
		If myVersion >= 251 Then
			'UPGRADE_WARNING: Couldn't resolve default property of object myInfos.ReadFromFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			myInfos.ReadFromFile(FromFile)
		End If
		'    myVersion = 251 ' [Titi 2.5.1] Changed to account for the new clsStat class
		Exit Sub
ErrorHandler: 
	...
	Public Sub SaveToFile(ByRef ToFile As Short)
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object myInfos.SaveToFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		myInfos.SaveToFile(ToFile)
	End Sub
	...
	Private Sub Class_Initialize_Renamed()
		...
		Me.RequiredInTome = True ' [Titi 2.6.0] default new creatures to NPC
		Me.Agressive = True
		Me.Guard = True
		Me.Friendly = False
		Me.Male = True
