...
Module modBD
	...
	Public gLibPath As String ' [Titi 2.6.0]
	Public gDebug As Boolean
	' [Titi 2.4.7] added for world settings creation option
	'UPGRADE_ISSUE: World object was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6B85A2A7-FE9F-4FBE-AA0C-CF11AC86A305"'
	...
	' [Titi 2.6.0] added possibility to change runes refresh rates and rarity
	Public intRare, intVeryRare, intCommon As Short
	Public strRarityPattern As String
	' [Titi 2.5.1] added to expand the list of body parts
	Public BodyType As Short
	' [Titi 2.5.1] added to tell the game what map is currently active
	Public MapTag As String
	
	Public DmgDice As String
	...
	Public Const bdNbTriggers As Short = 79
	...
	Public Const bdShowNewStat As Short = 79 ' [Titi 2.4.9b]
	
	'[Titi 2.6.0] preferred interface (per world setting)
	Public strInterfaceName As String
	...
	Public Function TriggerName(ByVal Index As Short) As String
		Select Case Index
			...
			Case bdShowNewStat ' [Titi 2.4.9b]
				TriggerName = "ShowNewStat"
			Case bdOnIgnore
	...
	Public Sub StatementToList(ByRef TomeX As Tome, ByRef AreaX As Area, ByRef TrigX As Trigger, ByRef StmtX As Statement, ByRef ListIndex() As Short, ByRef List() As String)
		...
		Select Case StmtX.Statement
			...
			Case 21 'CopyTile 'Text' To ([Context.Var],[Context.Var],[List])
				...
				List(0, 5) = "TopFlip"
				ListIndex(0) = 5
				' [Titi 2.5.1] help for the creator: replace 'Text' above by a list of possible tiles
				c = -1
				For	Each AreaZ In Tome.Areas
					For	Each MapX In AreaZ.Plot.Maps
						If MapTag = "M" & MapX.Index Then
							' found the map we're working on
			...
			Case 48 'DialogBuySell [List] At [Context.Var]
				...
				List(0, 2) = "TriggerNow"
				ListIndex(0) = 2
			Case 49 'Reserved [Titi 2.5.1] TargetEncounterInArea [List] Map [List] Named [List]
				c = -1 : i = -1 : j = -1
				For	Each AreaZ In Tome.Areas
					c = c + 1
					List(0, c) = AreaZ.Name
					If c = StmtX.B(0) Then ' list of maps based on current area
						If AreaZ.IsLoaded = False Then
							i = FreeFile
							FileOpen(i, Tome.LoadPath & "\" & AreaZ.FileName, OpenMode.Binary)
							AreaZ.Plot = New Plot
							AreaZ.Plot.ReadFromFile(i)
							FileClose(i)
							AreaZ.IsLoaded = True
							i = -1
						End If
						For	Each MapX In AreaZ.Plot.Maps
							i = i + 1
							List(1, i) = MapX.Name
							If i = StmtX.B(1) Then ' list of encounters based on current map
								For	Each EncounterX In MapX.Encounters
									j = j + 1
									List(2, j) = EncounterX.Name
								Next EncounterX
								ListIndex(2) = j
							End If
						Next MapX
			...
			Case 67 'TargetEncounter 'Text'
				' [Titi 2.5.1] help for the creator: replace 'Text' above by a list of possible encounters
				c = -1
				For	Each AreaZ In Tome.Areas
					For	Each MapX In AreaZ.Plot.Maps
						If MapTag = "M" & MapX.Index Then
	...
	Public Function StatementToText(ByRef TomeX As Tome, ByRef AreaX As Area, ByRef TrigX As Trigger, ByRef StmtX As Statement, ByRef Text As String) As Short
		...
		Select Case StmtX.Statement
			...
			Case 12 'Put [Context.Var] [Op] [Context.Var] Into [Context.Var]
				...
				If My.Application.Info.AssemblyName Like "Creator" Then
					If frmTriggerProp.IsReadOnly(StmtX.B(5), StmtX.B(6)) Then
						Text = "Coding error: " & ContextToText(StmtX.B(5)) & "." & VarToText(StmtX.B(5), StmtX.B(6)) & " is a read-only property! "
					End If
				End If
				Text = Text & "Put " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " " & OpToText(StmtX.B(2)) & " " & ContextToText(StmtX.B(3)) & "." & VarToText(StmtX.B(3), StmtX.B(4)) & " Into " & ContextToText(StmtX.B(5)) & "." & VarToText(StmtX.B(5), StmtX.B(6))
			Case 13 'Set [List] = [List]
			...
			Case 22 'CopyText 'Text' Into [Context.Var]
				...
				'UPGRADE_WARNING: App property App.EXEName has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
				If My.Application.Info.AssemblyName Like "Creator" Then
					If frmTriggerProp.IsReadOnly(StmtX.B(0), StmtX.B(1)) Then
						Text = "Coding error: " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " is a read-only property! "
					End If
				End If
			...
			Case 23 'Runes ([List],[List],[List],[List],[List],[List]) Fail Save
				' [Titi 2.4.9b] Customization of the magic system used in the world
			...
			Case 36 'DialogShow [List] 'Text' Says 'Text'
				' [Titi 2.4.6] removed the second "Briefline", but only for display (to keep compatibility with existing tomes)
			...
			Case 38 'DialogAccept [Context.Var]
				' [Titi 2.6.0] Don't authorize impossibilities (read-only properties cannot be at the left of the = sign)
				Text = ""
				If frmTriggerProp.IsReadOnly(StmtX.B(0), StmtX.B(1)) Then
					Text = "Coding error: " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " is a read-only property! "
				End If
				Text = Text & "DialogAccept " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1))
			Case 39 'DialogDice 'Text' [List] Says 'Text' Into [Context.Var]
				Text = ""
				If frmTriggerProp.IsReadOnly(StmtX.B(0), StmtX.B(1)) Then
					Text = "Coding error: " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " is a read-only property! "
				End If
				Text = Text & "DialogDice " & Chr(34) & BreakText(TextQuote(StmtX.Text), 1) & Chr(34) & " " & List(0, StmtX.B(2)) & " Says " & Chr(34) & BreakText(TextQuote(StmtX.Text), 2) & Chr(34) & " Into " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1))
			Case 40 'DialogHide
				Text = "DialogHide"
			...
			Case 49 'Reserved [Titi 2.5.1] TargetEncounterInArea [List] Map [List] Named [List]
				Text = "TargetEncounterInArea " & Chr(34) & List(0, StmtX.B(0)) & Chr(34) & " Map " & Chr(34) & List(1, StmtX.B(1)) & Chr(34) & " Named " & Chr(34) & List(2, StmtX.B(2)) & Chr(34)
			Case 50 'ExecTrigger [List]
			...
			Case 51 'DialogAcceptText [Context.Var]
				...
				If frmTriggerProp.IsReadOnly(StmtX.B(0), StmtX.B(1)) Then
					Text = "Coding error: " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " is a read-only property! "
				End If
				Text = Text & "DialogAcceptText " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1))
			Case 52 'CombatStart
				Text = "CombatStart"
			Case 53 'Let [Context.Var] = [Context.Var]
				' [Titi 2.6.0] Don't authorize impossibilities (read-only properties cannot be at the left of the = sign)
				...
				If My.Application.Info.AssemblyName Like "Creator" Then
					...
					End If
				End If
				Text = Text & "Let " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " = " & ContextToText(StmtX.B(3)) & "." & VarToText(StmtX.B(3), StmtX.B(4))
			Case 54 '' Text
	...
	Public Function VarToText(ByRef Context As Short, ByRef Var As Short) As String
		...
		Dim c As Short
		Text = Right(strRunesList, Len(strRunesList) - 5) ' get rid of "List="
		ReDim sRune(20) 'sRune(intNbRunes)
		For c = 1 To intNbRunes - 1
		...
		Select Case Context
			...
			Case 1 ' Map
				Select Case Var
					...
					Case 11 To 30
						'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Runes. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						VarToText = IIf(WorldNow.Runes, sRune(Var - 10), "Rune" & sRune(Var - 10))
					...
					Case 35 : VarToText = "CanCamp" ' [Titi 2.5.1]
				End Select
			Case 2 To 6 ' Tile
				Select Case Var
			...
			Case 7 To 11 ' Encounter
				Select Case Var
					...
					Case 7 : VarToText = "CanCamp" '"<Reserved>" [Titi 2.5.1]
			...
			Case 12 To 16 ' Creature
				Select Case Var
					...
					Case 39 : VarToText = "ProtectMind%"
						'Case 40 To 45: VarToText = "<Reserved>" [Titi 2.5.1]
					Case 40 : VarToText = "AgeYear"
					Case 41 : VarToText = "AgeMoon"
					Case 42 : VarToText = "AgeCycle"
					Case 43 : VarToText = "AgeTurn"
					...
					Case 75
						...
						VarToText = IIf(WorldNow.Magic = "", "Eternium", WorldNow.Magic)
					...
					Case 133
						'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Magic. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						VarToText = IIf(WorldNow.Magic <> vbNullString, WorldNow.Magic, "Eternium") & "Max"
			...
			Case 17 To 21 ' Item
				Select Case Var
					...
					Case 4 : VarToText = "IsTiny"
					Case 5 : VarToText = "IsSmall"
					Case 6 : VarToText = "IsNormalSize"
					Case 7 : VarToText = "IsLarge"
					Case 8 : VarToText = "IsHuge"
					Case 9 : VarToText = "FitsAllSizes"
					Case 10 : VarToText = "Level"
					Case 11 To 15 : VarToText = "<Reserved>"
			...
			Case 28 ' Global
				Select Case Var
					...
					Case 9 To 28
						'UPGRADE_WARNING: Couldn't resolve default property of object WorldNow.Runes. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						VarToText = IIf(WorldNow.Runes, sRune(Var - 8), "Rune" & sRune(Var - 8))
	...
	Function PointIn(ByRef XX As Short, ByRef YY As Short, ByRef LeftX As Short, ByRef TopY As Short, ByRef WidthX As Short, ByRef HeightY As Short) As Short
		' Determines if a value is inside a rectangle
	...
	Public Function SetUpBodyType(ByRef Index As Short) As String
		Select Case Index
			Case 0
				SetUpBodyType = "Humanoid"
			Case 1
				SetUpBodyType = "Flying"
			Case 2
				SetUpBodyType = "Ship"
			Case 3
				SetUpBodyType = "Special"
			Case Else
				SetUpBodyType = "Undefined"
		End Select
	End Function
	...
	Public Function SetUpArmorType(ByRef Index As Short) As String
		Select Case Index
			...
			Case 10
				SetUpArmorType = "Neck"
				' [Titi 2.5.1] Body parts for birds/dragons (above * kept only for compatibility purposes with monsters created in a previous version
			Case 11
				SetUpArmorType = "Mandible"
			Case 12
				SetUpArmorType = "Tail"
			Case 13
				SetUpArmorType = "Body"
			...
			Case 21
				SetUpArmorType = "Neck"
				' [Titi 2.5.1] Body parts for ships
			Case 22
				SetUpArmorType = "Wing"
			Case 23
				SetUpArmorType = "Rudder"
			Case 24
				SetUpArmorType = "Hull"
			Case 25
				SetUpArmorType = "Bridge"
			Case 26
				SetUpArmorType = "Deck"
			Case 27
				SetUpArmorType = "Turret"
			Case 28
				SetUpArmorType = "Masts"
			Case 29
				SetUpArmorType = "Crew quarters"
			Case 30
				SetUpArmorType = "Steering"
			Case 31
				SetUpArmorType = "Cargo bay"
			Case 32
				SetUpArmorType = "Forecastle"
				' body parts for special
			Case 33
				SetUpArmorType = "Branches"
			Case 34
				SetUpArmorType = "Roots"
			Case 35
				SetUpArmorType = "Trunk"
			Case 36
				SetUpArmorType = "Bark"
			Case 37
				SetUpArmorType = "Foliage"
			Case 38
				SetUpArmorType = "Tentacle"
			Case 39
				SetUpArmorType = "Claws"
			Case 40
				SetUpArmorType = "Sting"
			Case 41
				SetUpArmorType = "Antenna"
			Case 42
				SetUpArmorType = "Trump"
			Case 43
				SetUpArmorType = "Lianes"
			Case Else
	...
	Public Sub InitializeRunes(ByRef strWorldName As String)
		...
		If Not (oFileSys.CheckExists(FileName, File)) Then
		...
		' [Titi 2.6.0] added the refresh rate and rarity of runes
		Text = LineInput(hndFile) ' fourth line = refresh rate
		Text = Right(Text, Len(Text) - 5) ' get rid of "Rate="
		intVeryRare = CShort(Val(Left(Text, InStr(Text, ",") - 1)))
		Text = Mid(Text, InStr(Text, ",") + 1)
		intRare = CShort(Val(Left(Text, InStr(Text, ",") - 1)))
		Text = Mid(Text, InStr(Text, ",") + 1)
		intCommon = CShort(Val(Text))
		strRarityPattern = LineInput(hndFile) ' fifth line = very rare | common runes
		strRarityPattern = Right(strRarityPattern, Len(strRarityPattern) - 7) ' get rid of "Rarity="
		Text = LineInput(hndFile) ' sixth line = runes pictures set
		FileClose(hndFile)
		...
		If GlobalInterfaceName <> "" Then
			...
			'UPGRADE_WARNING: Couldn't resolve default property of object oFileSys.CheckExists. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			If oFileSys.CheckExists(gAppPath & "\Roster\" & sWorld & "\" & Text, File) Then
				'UPGRADE_WARNING: Couldn't resolve default property of object frmMain.picRuneSet. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				'UPGRADE_WARNING: Couldn't resolve default property of object sWorld. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				frmMain.picRuneSet.Picture = System.Drawing.Image.FromFile(gAppPath & "\Roster\" & sWorld & "\" & Text)
			Else
				' default=Eternia set
				'            frmMain.picRuneSet.Picture = LoadPicture(gAppPath & "\data\Interface\" & GlobalInterfaceName & "\RuneSet.bmp")
				'UPGRADE_WARNING: Couldn't resolve default property of object frmMain.picRuneSet. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				frmMain.picRuneSet.Picture = System.Drawing.Image.FromFile(gDataPath & "\Interface\" & GlobalInterfaceName & "\RuneSet.bmp")
			End If
		End If
	End Sub
	
	' [Titi 2.5.1] moved from frmMonsProp
	Public Function DamagesOnAttack(ByRef TriggerX As Trigger) As Short
		Dim StmtX As Statement
		Dim DmgDice, AttDice As String
		Dim blnRoll As Boolean
		Dim intCoefDamage, intCoefAttack As Short
		Const bdAvgDef As Short = 13
		blnRoll = False
		For	Each StmtX In TriggerX.Statements
			With StmtX
				DmgDice = "0d0" : AttDice = "0d0"
				If .Statement = 27 Then
					' CombatApplyDamage
					DmgDice = VarToText(.B(3), .B(4))
				ElseIf .Statement = 28 Then 
					'CombatRollAttack
					AttDice = VarToText(.B(0), .B(1))
					blnRoll = True
				ElseIf .Statement = 29 Then 
					' CombatRollDamage
					DmgDice = VarToText(.B(0), .B(1))
				ElseIf .Statement = 62 Then 
					' CombatAttackWithWeapon
					' already done
				ElseIf .Statement = 63 Then 
					' CombatAttackWithSpecial
					DmgDice = VarToText(.B(5), .B(6))
				End If
				If DmgDice <> "0d0" Then
					' case of CombatRollAttack
					If blnRoll Then
						blnRoll = False
						intCoefAttack = DiceDamage(AttDice) / bdAvgDef
					Else
						intCoefAttack = 100
					End If
					intCoefDamage = intCoefDamage + DiceDamage(DmgDice) / 100 * intCoefAttack
				End If
				If AttDice <> "0d0" Then
					intCoefAttack = intCoefAttack + DiceDamage(DmgDice)
				End If
			End With
			
		Next StmtX
		DamagesOnAttack = intCoefDamage
	End Function
	
	Public Function NbAttacksPerTurn(ByRef TriggerX As Trigger, ByRef CreatureX As Creature) As Short
		Dim StmtX As Statement
		Dim intRatio, intNbActions, intStmt As Short
		intStmt = 0
		For	Each StmtX In TriggerX.Statements
			With StmtX
				If .Statement = 12 Then
					intStmt = intStmt + 1
					'  "Put " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " " & OpToText(StmtX.B(2)) & " " & ContextToText(StmtX.B(3)) & "." & VarToText(StmtX.B(3), StmtX.B(4)) & " Into " & ContextToText(StmtX.B(5)) & "." & VarToText(StmtX.B(5), StmtX.B(6))
					If ContextToText(.B(0)) & "." & VarToText(.B(0), .B(1)) = "CreatureNow.ActionPoints" Then
						intNbActions = intNbActions + 100 * (CreatureX.ActionPointsMax / Val(CStr(.B(4))))
					End If
				ElseIf .Statement = 53 Then 
					'  "Let " & ContextToText(StmtX.B(0)) & "." & VarToText(StmtX.B(0), StmtX.B(1)) & " = " & ContextToText(StmtX.B(3)) & "." & VarToText(StmtX.B(3), StmtX.B(4))
					If ContextToText(.B(0)) & "." & VarToText(.B(0), .B(1)) = "CreatureNow.ActionPoints" Then
						If Val(CStr(.B(4))) = 0 Then
							intStmt = intStmt + 1
							intNbActions = intNbActions + 100
						Else
							intStmt = intStmt + 1
							intNbActions = intNbActions + 100 * (CreatureX.ActionPointsMax / Val(CStr(.B(4))))
						End If
			...
			End With
		Next StmtX
		If intStmt > 1 Or intNbActions = 0 Then intNbActions = 100 ' this means that several attacks are done, but the damages are already totalled
		NbAttacksPerTurn = intNbActions
	End Function
	
	Public Function LookForAmmo(ByRef ItemY As Item, ByRef ShootType As Byte, ByRef DamageBonus As Short) As String
		Dim ItemX As Item
		If ItemY.Items.Count() > 0 Then
			' case of the quiver
			For	Each ItemX In ItemY.Items
				LookForAmmo(ItemX, ShootType, DamageBonus)
				If DmgDice <> "" Then Exit Function ' found corresponding ammo, no need to check other items
			Next ItemX
		Else
			If ShootType = ItemY.ShootType Then
				' yep, proceed
				DmgDice = VarToText(31, ItemY.Damage - 1) ' -1 to account for the "none" possibility
				If InStr(DmgDice, "+") <> 0 Then
					DmgDice = Left(DmgDice, InStr(DmgDice, "d")) & Str(Val(Right(DmgDice, Len(DmgDice) - InStr(DmgDice, "+"))) + DamageBonus)
				ElseIf DamageBonus <> 0 Then 
					DmgDice = DmgDice & "+" & Str(DamageBonus)
				End If
				' nope, next candidate
			End If
			LookForAmmo = DmgDice
	...
	Public Function DiceDamage(ByRef DmgDice As String) As Short
		Dim FaceDie, NbDice, Bonus As Short
		NbDice = Val(Left(DmgDice, InStr(DmgDice, "d")))
		FaceDie = Val(Right(DmgDice, Len(DmgDice) - InStr(DmgDice, "d")))
		If InStr(DmgDice, "+") <> 0 Then
			Bonus = Val(Right(DmgDice, Len(DmgDice) - InStr(DmgDice, "+")))
		Else
			Bonus = 0
		End If
		DiceDamage = 100 * (NbDice * ((1 + FaceDie) / 2) + Bonus)
	End Function
	
	Public Function GetWeaponDamage(ByRef CreatureX As Creature, ByRef intCoefAtt As Short, ByRef intCoefDef As Short) As Short
		Dim ItemX, ItemY As Item
		Dim intCoefDamageWeapons As Short
		For	Each ItemX In CreatureX.Items
			If ItemX.IsReady Then
				' magic items may give bonuses
				intCoefAtt = intCoefAtt + ItemX.AttackBonus
				' intCoefDef = intCoefDef + ItemX.Defense
				' if it's an armor, it's already accounted for. ItemX.Family doesn't allow to determine if it's an armor, so forget it
			End If
			If ItemX.InHand Then
				If ItemX.IsShooter Then
					' then damage is caused by arrows
					For	Each ItemY In CreatureX.Items
						LookForAmmo(ItemY, (ItemX.ShootType), (ItemX.DamageBonus))
						If DmgDice <> "" Then
							' found arrows
							intCoefAtt = intCoefAtt + ItemY.AttackBonus
							intCoefDef = intCoefDef + ItemY.Defense
							Exit For ' no need to continue
						End If
					Next ItemY
				Else
					DmgDice = VarToText(31, ItemX.Damage - 1)
				End If
				intCoefDamageWeapons = (DiceDamage(DmgDice) + CreatureX.DamageBonus) * (CreatureX.ActionPointsMax / ItemX.ActionPoints)
				' intCoefDamageWeapons is the average damage inflicted by the creature during its turn with the weapon in hand
			End If
		Next ItemX
		GetWeaponDamage = intCoefDamageWeapons
	End Function
	
	Public Function GetTriggersAttack(ByRef CreatureX As Creature) As Short
		Dim blnCastsSpells As Object
		Dim TriggerX, TriggerY As Trigger
		Dim intCoefDamageSpecial As Short
		For	Each TriggerX In CreatureX.Triggers
			If TriggerName(TriggerX.TriggerType) Like "On-Attack" Then
				intCoefDamageSpecial = DamagesOnAttack(TriggerX) / 100 * NbAttacksPerTurn(TriggerX, CreatureX)
				If intCoefDamageSpecial = 0 Then
					' probably the On-Attack trigger calls subtriggers - see Eye of Nuros in TLB
					For	Each TriggerY In TriggerX.Triggers
						' case of the spells
						'UPGRADE_WARNING: Couldn't resolve default property of object blnCastsSpells. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						If LCase(Right(TriggerY.Name, 5)) = "spell" Then blnCastsSpells = True
						intCoefDamageSpecial = intCoefDamageSpecial + DamagesOnAttack(TriggerY) / 100 * NbAttacksPerTurn(TriggerY, CreatureX)
					Next TriggerY
				End If
			End If
		Next TriggerX
		GetTriggersAttack = intCoefDamageSpecial
	End Function
	
	Public Function GetDefCoef(ByRef CreatureX As Creature, ByRef intArmorProtection As Object, ByVal bdAvgStats As Object) As Short
		Dim intCoefDef, i As Short
		With CreatureX
			For i = 0 To 7
				'UPGRADE_WARNING: Couldn't resolve default property of object intArmorProtection. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				intArmorProtection = intArmorProtection + .ResistanceWithArmor(i) + .Resistance(i) + .ResistanceBonus(i)
			Next i
			'UPGRADE_WARNING: Couldn't resolve default property of object intArmorProtection. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			intArmorProtection = 100 + intArmorProtection / 8
			intCoefDef = .Defense '+ .DefenseBonus
		End With
		GetDefCoef = intCoefDef
	End Function
	
	Public Function GetXPValue(ByRef CreatureX As Creature) As Short
		Dim i As Short
		Dim intXP As Integer
		Dim ItemX, ItemY As Item
		Dim TriggerX As Trigger
		Dim StmtX As Statement
		Dim intCoefAtt, intCoefHealth, intCoefStats, intCoefDef As Short
		Dim intCoefDamageWeapons, intAvgDamage, intCoefDamageSpecial As Short
		Dim TriggerY As Trigger
		Dim intXPmin, intArmorProtection, intXPmax As Short
		Dim blnCastsSpells As Boolean
		Dim sngLevel As Single
		Const bdBaseHP As Short = 13 ' average health of a 1st level NPC
		Const bdAvgStats As Short = 13
		Const bdAvgDef As Short = 13 ' average min roll on 1d20 needed to hit
		With CreatureX
			blnCastsSpells = .IsSpellCaster
			intCoefStats = 100 * ((.Agility + .Will + .Strength) / 3 / bdAvgStats)
			intCoefAtt = .StrengthBonus
			intCoefDef = GetDefCoef(CreatureX, intArmorProtection, bdAvgStats)
			' based on Crosscut's leveling spreadsheet
			intAvgDamage = (.Level + 1) * (.Level + 2) / 2 + 2
			intCoefHealth = .Level * (bdBaseHP + (10 * (.Level - 5) + 2) * (-CShort(.Level > 4)))
			intCoefHealth = .HPMax / intCoefHealth * 100
			' incorporation of other parameters in the formula
			' see http://www.runesword.com/forums/viewtopic.php?t=84&start=30
			' now, it depends on the weapons
			intCoefDamageWeapons = GetWeaponDamage(CreatureX, intCoefAtt, intCoefDef)
			' Damages caused by weapons now done. What about special attacks?
			intCoefDamageSpecial = GetTriggersAttack(CreatureX)
			' based on Crosscut's leveling spreadsheet (valid only up to level 10)
			intXP = .Level * 90 + 5 * (.Level - 5) * CShort(1 - CShort(.Level < 3))
			' now, adjust this base XP value according to the variations against the averages
			' note that the formula as it is doesn't account for particulars (regeneration, cause fear, poison, etc...)
			intCoefAtt = 5 * (20 + intCoefAtt - bdAvgDef) ' probability of a hit against standard opponent
			intCoefDef = 5 * (20 - intCoefDef) ' probability of being hit by a standard opponent
			intXP = intXP * ((intCoefDamageWeapons + intCoefDamageSpecial / 2) / 100 / intAvgDamage) * (intCoefAtt / (20 - bdAvgDef) / 5) * (intCoefDef / 100 * intArmorProtection / 100) * (intCoefHealth / 100) * (intCoefStats / 100)
			If blnCastsSpells Then intXP = 1.5 * intXP
			' just for "realism", let's compute a min and max XP value
			sngLevel = .Level * (intCoefAtt / (20 - bdAvgDef) / 5) * (intCoefDef / 100 * intArmorProtection / 100) * (intCoefHealth / 100)
			If sngLevel < .Level - 1 Then sngLevel = .Level - 1
			If sngLevel > .Level + 2 Then sngLevel = .Level + 2
			...
			intXPmax = ((sngLevel + 1) * Int((sngLevel + 1) / 2 + 0.5) + ((sngLevel + 1) / 2) * (1 - ((sngLevel + 1) Mod 2))) * 1000 / 22
			'UPGRADE_WARNING: Mod has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
			intXPmin = ((sngLevel) * Int((sngLevel) / 2 + 0.5) + ((sngLevel) / 2) * (1 - ((sngLevel) Mod 2))) * 1000 / 28
		End With
		If intXP > intXPmax Then intXP = intXPmax
		If intXP < intXPmin Then intXP = intXPmin
		GetXPValue = intXP
	End Function
End Module