...
Friend Class Item
	...
	' [Titi 2.4.9b] If true, item not examined --> display warning message when using it for the first time
	...
	'       12-15 - Undefined
	'   Bit 5-10    - Size required to handle the item [Titi 2.4.9b]
	'       16 - Tiny, 32 - Small, 64 - Normal, 128 - Large, 256 - Huge, 512 - Any size can use
	'   Bit 11-14   - Not Used
	'   Bit 15     - On = IsReady
	...
	Public ReadOnly Property Hint() As String
		Get
			...
			ElseIf Me.Damage > 0 Then  ' Potential Weapon
				...
				End If
			End If
			' [Titi 2.4.9b] Don't show too much if item not examined!
			Hint = Me.Name & " * Bulk " & Me.Bulk & "  Wgt " & Me.Weight & IIf(Me.Examined, Hint, "")
		End Get
	End Property
	...
	Public Property Examined() As Short
		Get ' [Titi 2.4.9b]
			Examined = (myNotUsed And &H1) > 0
		End Get
		Set(ByVal Value As Short) ' [Titi 2.4.9b]
			If Value Then
				myNotUsed = myNotUsed Or &H1
			Else
				myNotUsed = myNotUsed And (Not &H1)
			End If
		End Set
	End Property
	
	
	Public Property NotUsed() As Short
		Get
			NotUsed = myNotUsed
		End Get
		Set(ByVal Value As Short)
	...
	Public Property MinLevel() As Short
		Get ' [Titi 2.4.9b]
			MinLevel = CShort(myNotUsed And &HFE) / 2
		End Get
		Set(ByVal Value As Short) ' [Titi 2.4.9b]
			' Wipe out the old value
			myNotUsed = (myNotUsed And &H1)
			' Set the new level
			myNotUsed = (myNotUsed Or Value * 2)
		End Set
	End Property
		...
		Get
			...
			Select Case (myShootType And &H3)
				Case 0 ' Short
					MinRange = 1
				Case 1 ' Medium
					MinRange = 2 ' [Titi 2.4.9b] was 1
				Case 2 ' Long
					MinRange = 4 ' [Titi 2.4.9b] was 3
				Case Else
					MinRange = 1
			End Select
		End Get
	...
	' [Titi 2.4.9b] added a size to the item
	
	Public Property SizeBit(ByVal Index As Short) As Short
		Get
			SizeBit = (myWearType And (2 ^ Index) > 0)
		End Get
		Set(ByVal Value As Short)
			If Value Then
				myWearType = myWearType Or (2 ^ Index)
			Else
				myWearType = myWearType And (Not (2 ^ Index))
			End If
		End Set
	End Property
	...
	Public Property Size() As Short
		Get
			Dim iTmp As Short
			iTmp = (myWearType And &H3F0)
			If iTmp > 512 Then iTmp = iTmp - 512
			Select Case iTmp
				Case 16
					Size = 1 'tiny
				Case 32
				...
				Case 256
					Size = 5 ' huge
				Case Else
					' undefined --> default to normal, usable by anybody
					Me.SizeBit(6) = 1
					Size = 3
					Me.SizeBit(9) = 1
			End Select
		End Get
		Set(ByVal Value As Short)
			' Wipe out the old size bits
			myWearType = (myWearType And &HF)
			' Set the new size
			Select Case Value
				Case 1
					Value = 16 'tiny
				...
				Case 5
					Value = 256 ' huge
			End Select
			myWearType = (myWearType Or Value)
		End Set
	End Property
	' [Titi 2.4.9b] end size addition
	...
	Public Property DamageType() As Short
		Get
			If myFamily > 4 And myFamily < 9 Or myFamily = 3 Then ' [Titi 2.4.9b] added the food family
	...
	Public Function RemoveMoney(ByRef AmountToRemove As Short) As Short
		...
		For	Each ItemX In Me.Items
			...
			If ItemX.RemoveMoney(AmountToRemove) <= 0 Then
		...
		If Me.IsMoney = True And AmountToRemove > 0 Then
			' [Titi 2.4.9b] made the calculations in double (with integers, we could have Take = 0...)
			If myValue > 0 Then
				'            Take = Int(AmountToRemove / (myValue / myCount))
				sChange = CSng(myValue) / CSng(myCount)
				Take = CDbl(AmountToRemove) / CDbl(sChange)
				If myBulk > 0 Then
					'                myBulk = myBulk - Take * Int(myBulk / myCount)
					myBulk = myBulk - CShort(Take * myBulk / myCount)
				End If
				If myWeight > 0 Then
					'                myWeight = myWeight - Take * Int(myWeight / myCount)
					myWeight = myWeight - CShort(Take * myWeight / myCount)
				End If
				If myCount > 0 Then
					myCount = myCount - IIf(Int(Take) = Take, Take, Int(Take) + 1)
				End If
				' [Titi 2.4.9b] What about change?
				'            myValue = myValue - AmountToRemove
				If Take <> Int(Take) Then
					Take = (Int(Take) + 1 - Take) * CDbl(sChange)
				Else
					Take = 0
				End If
				myValue = CShort(CSng(myCount) * sChange)
				AmountToRemove = AmountToRemove - myValue
			End If
			'        If myValue < 1 Then
			'            RemoveMoney = True
			If myValue <= 0 Then
				RemoveMoney = myValue ' [Titi 2.4.9b] means there isn't enough money in that item, so we need to deduct the balance from another one!
			Else
				RemoveMoney = IIf(Take = 0, 1, CShort(Take + 1))
			End If
		End If
	End Function
	...
	Public Sub Copy(ByRef FromItem As Item)
		...
		myPic = FromItem.Pic
		' [Titi 2.4.9b] myNotUsed has found a use now (Examined? flag and Required level to use)
		myNotUsed = FromItem.NotUsed
