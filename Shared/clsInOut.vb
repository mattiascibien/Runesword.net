...
Err_Handler: 
	...
	Public Function Delete(ByRef ioName As String, ByRef ioType As IOActionType, Optional ByRef Force As Boolean = False) As Boolean
		...
		Else
			'        ' [Titi 2.5.0] create a back-up copy, just in case...
			'        sBackup = gAppPath & "\world\" & WorldNow.Name & "\" & AssignTmpName(ioType) '& "\"
			'        If Copy(ioName, sBackup, ioType, Force) = True Then
...
Exit_Function: 
		Delete = True
		'    If ioType = Folder Then
		'        Call oFs.DeleteFolder(sBackup, Force) ' [Titi 2.5.0] everything worked well, so backup not necessary any more
		'    End If
		Exit Function
Err_Handler: 
		'    If Err.Number = 70 Then '[Titi 2.5.0] Folder in use - unable to delete!
		'        If Copy(sBackup, ioName, ioType, Force) = True Then ' put the original files in place
		'            Call oFs.DeleteFolder(ioName , Force)
		'        End If
		'    End If
		Err.Clear()
	...
	Public Function Copy(ByRef Src As String, ByRef Dest As String, ByRef ioType As IOActionType, Optional ByRef Forced As Boolean = True) As Object
		...
		Else
			'UPGRADE_WARNING: Couldn't resolve default property of object oFs.DeleteFolder. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			Call oFs.DeleteFolder(Src, Forced)
		End If
Exit_Function: 
