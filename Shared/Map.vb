...
Friend Class Map
	...
	' PictureFile is the file name of the graphic. Looks in \tome first, then in \roster\<worldname>, then in /data.
	...
	' Bit3  - On=Camping possible everywhere, Off=Camping only in encounters with the same flag set
	' Bit4  Not Used
	' Bit5-8 (MapStyle)
	'       0 - Town Map
	'       1 - Wilderness Map
	...
	' Set of TileSet names for the TileSet for this map. Just used as a collection point for tiles.
	...
	' Time between rests
	Private myInterval As Short
	' Chances of wandering monsters appearances
	Private myHazardChances As Short
	' Selection of wandering monsters
	...
	Public Property CanCamp() As Short
		Get
			CanCamp = (myGenerateUponEntry And &H4) > 0
		End Get
		Set(ByVal Value As Short)
			If Value Then
				myGenerateUponEntry = myGenerateUponEntry Or &H4
			Else
				myGenerateUponEntry = myGenerateUponEntry And (Not &H4)
			End If
		End Set
	End Property
	...
	Public Property Hazard() As Short
		Get
			Hazard = myHazardChances
		End Get
		Set(ByVal Value As Short)
			myHazardChances = Value
		End Set
	End Property
	...
	Public Function AddTheme() As Theme
		' Adds a Theme and returns a reference to that Theme
		Dim c As Short
		Dim ThemeX As Theme
		' Find new available unused index identifier
		c = 1
	...
	Public Function AddTileSet() As TileSet
		...
		' Find new available unused index identifier
		c = 1
		' [Titi 2.5.1] Not good... What if some tilesets have been deleted before we add another one, hm?
...
ErrorHandler: 
		Dim i As Short
		If Err.Number = 5 Then ' [Titi 2.6.0] in previous versions of RS, tilesets only had numeric keys
			c = Val(Mid(DeleteKey, 2))
			' [Titi 2.6.1] Delete/Cut may have messed up with the indices...
			For i = 1 To myTileSets.Count()
				'UPGRADE_WARNING: Couldn't resolve default property of object myTileSets(i).Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				If c = myTileSets.Item(i).Index Then c = i
			Next 
			Resume Next
		End If
	End Sub
	...
	Public Sub ReadFromFile(ByRef FromFile As Short)
		...
		' [Titi 2.5.1] wandering monsters options
		If myVersion >= 251 Then
			'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
			FileGet(FromFile, myInterval)
			'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
			FileGet(FromFile, myHazardChances)
			Reading = "Wandering Monster"
			'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
			FileGet(FromFile, Cnt)
			For c = 1 To Cnt
				CreatureX = New Creature
				CreatureX.ReadFromFile(FromFile)
				myCreatures.Add(CreatureX, "X" & CreatureX.Index)
			Next c
		End If
		' Read Runes
		...
		For c = 1 To Cnt
			...
			myTileSets.Add(TileSetX) ', Key:="W" & TileSetX.Index
		Next c
		' Read Tiles
		Reading = "Tile"
		'UPGRADE_WARNING: Get was upgraded to FileGet and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
...
ErrorHandler: 
		...
		If Err.Number = 457 And Reading <> "" Then
			Select Case Reading
				...
				Case "Wandering Monster"
					oErr.logError(Reading & " index conflict: " & CreatureX.Index)
			End Select
		Else
	...
	Public Sub Copy(ByRef FromMap As Map)
		...
		Next c
		' [Titi 2.5.1] copy wandering monsters options
		If myVersion >= 251 Then
			myCreatures = New Collection
			For c = 1 To FromMap.Creatures.Count()
				'UPGRADE_WARNING: Couldn't resolve default property of object FromMap.Creatures().Index. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				CreatureX = Me.AddCreatureAsIndex(FromMap.Creatures.Item(c).Index)
				'UPGRADE_WARNING: Couldn't resolve default property of object FromMap.Creatures(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				CreatureX.Copy(FromMap.Creatures.Item(c))
			Next c
			myInterval = FromMap.Interval
			myHazardChances = FromMap.Hazard
		End If
	End Sub
	...
	Public Sub SaveToFile(ByRef ToFile As Short)
		...
		' [Titi 2.5.1] wandering monsters options
		'UPGRADE_WARNING: Put was upgraded to FilePut and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
		FilePut(ToFile, myInterval)
		'UPGRADE_WARNING: Put was upgraded to FilePut and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
		FilePut(ToFile, myHazardChances)
		'UPGRADE_WARNING: Put was upgraded to FilePut and has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="9B7D5ADD-D8FE-4819-A36C-6DEDAF088CC7"'
	...
	Private Sub Class_Initialize_Renamed()
		...
		' [Titi 2.5.1] by default, no camp allowed (to get the usual behavior in older tomes)
		Me.CanCamp = False
	End Sub
	Public Sub New()
		MyBase.New()
