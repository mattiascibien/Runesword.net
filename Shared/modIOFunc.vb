...
Module modIOFunc
	...
	Public Sub PlayMusic(ByVal SongName As String, ByRef frm As System.Windows.Forms.Form, Optional ByRef AlternatePath As Object = Nothing)
        ...
        FileName = modBD.Tome.FullPath & "\" & SongName
		If oFileSys.CheckExists(FileName, clsInOut.IOActionType.File) Then GoTo PlaySong
		'UPGRADE_WARNING: Couldn't resolve default property of object Tome.FullPath. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        FileName = modBD.Tome.FullPath & "\Combat\" & SongName
		If oFileSys.CheckExists(FileName, clsInOut.IOActionType.File) Then GoTo PlaySong
		'UPGRADE_WARNING: Couldn't resolve default property of object Tome.FullPath. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        FileName = modBD.Tome.FullPath & "\Adventure\" & SongName
		If oFileSys.CheckExists(FileName, clsInOut.IOActionType.File) Then GoTo PlaySong
...
Err_Handler: 
	...
	' [Titi 2.6.0] added to make worlds and tomes self installable
	Public Function InstallPackage(ByRef strName As String) As Short
		Dim ext As String
		Dim oFile, File As Object
		ext = Right(strName, 3)
		If ext = "rsp" Or ext = "exe" Then
			oFile = CreateObject("Scripting.FileSystemObject")
			'UPGRADE_WARNING: Couldn't resolve default property of object oFile.GetFile. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			File = oFile.GetFile(strName)
			'UPGRADE_WARNING: Couldn't resolve default property of object File.Name. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			File.Name = Left(File.Name, Len(File.Name) - 3) & "exe"
			InstallPackage = Shell(Left(strName, Len(strName) - 3) & "exe /S /D=" & gAppPath, AppWinStyle.Hide)
			Kill(Left(strName, Len(strName) - 3) & "exe")
		Else
			InstallPackage = 0
		End If
		If InstallPackage = 0 Then
			oErr.logError("Unable to unpack " & strName)
		End If
	End Function
	...
	Public Sub PlayMusicRnd(ByRef Style As RNDMUSICSTYLE, ByRef frm As System.Windows.Forms.Form)
		...
		If GlobalMusicState = 1 And GlobalMusicRandom = 1 Then
            ...
            sPath = modBD.Tome.LoadPath & "\" & IIf(Style, "Combat", "Adventure")
	...
	Public Sub PlayClickSnd(ByRef ClickType As ClickType)
		...
		If GlobalMouseClick = 1 And ClickType <= ClickType.ifClicklow Then
			Select Case ClickType
				Case ClickType.ifClickMainMenu ' [Titi 2.6.0]
					sSound = "click5.wav"
			...
			If sSound <> vbNullString Then
                ...
                Call PlaySoundFile(gDataPath & "\interface\" & GlobalInterfaceName & "\" & sSound, modBD.Tome, True)
			End If
		Else
			Select Case ClickType
			...
			If sSound <> vbNullString Then
                ...
                Call PlaySoundFile(gDataPath & "\stock\" & sSound, modBD.Tome, True, 5, &H10)
			End If
		End If
	End Sub
