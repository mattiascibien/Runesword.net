...
Friend Class World
	...
	' [Titi] Added for 2.4.9b
	' Name of the magic property
	Private myMagic As String
	Private myRunes As Boolean ' False = Eternia settings; True = doesn't use the runes system
	Private myMagicFormula As String
	Private myMagicPerLevel As Short
	Private myRandomizeMagic As Boolean
	' [Titi 2.4.9b] end of the magic changes
	...
	Public Property Runes() As Boolean
		Get
			Runes = myRunes
		End Get
	...
	Public Property Formula() As String
		Get
			Formula = myMagicFormula
		End Get
		Set(ByVal Value As String)
			myMagicFormula = Value
		End Set
	End Property
	...
	Public Property MagicPerLevel() As Short
		Get
			MagicPerLevel = myMagicPerLevel
		End Get
		Set(ByVal Value As Short)
			myMagicPerLevel = Value
		End Set
	End Property
	
	
	Public Property RandomizeMagic() As Boolean
		Get
			RandomizeMagic = myRandomizeMagic
		End Get
		Set(ByVal Value As Boolean)
			myRandomizeMagic = Value
		End Set
	End Property
		...
		Set(ByVal Value As Boolean)
			...
			If Value And Not bIsLoaded Then
				...
				For c = 1 To Kingdoms
					...
					' [Titi 2.5.1] added for the male/female distinction
					If Text = "0" Then
						' then, the world includes male & female pictures sets
						'UPGRADE_WARNING: Couldn't resolve default property of object fReadValue(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						lResult = fReadValue(FileName, "Kingdom" & c, "PictureFilesFemale", "S", "0", Text, 8192)
						AddPictureGender(False, Text, KingdomX, PictureX)
						'UPGRADE_WARNING: Couldn't resolve default property of object fReadValue(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						lResult = fReadValue(FileName, "Kingdom" & c, "PictureFilesMale", "S", "0", Text, 8192)
					End If
					' [Titi 2.5.1] moved to sub AddPictureGender
					AddPictureGender(True, Text, KingdomX, PictureX)
					'        Text = Text + ","
					'        i = InStr(1, Text, ",")
					'        k = 0
					'        Do While i > 0
					'            Set PictureX = KingdomX.AddPictureFile
	...
	Public Sub AddPictureGender(ByRef bIsMale As Boolean, ByRef Text As String, ByRef KingdomX As Kingdom, ByRef PictureX As Factoid)
		Dim k, i As Short
		Text = Text & ","
		i = InStr(1, Text, ",")
		k = 0
		Do While i > 0
			...
			' the ??? and *** will definitely not be used in a file name, so used to detect male/female pictures sets
			PictureX.Text = IIf(bIsMale, "???", "***") & Mid(Text, k + 1, i - k - 1)
			k = i
			i = InStr(k + 1, Text, ",")
		Loop 
	End Sub
